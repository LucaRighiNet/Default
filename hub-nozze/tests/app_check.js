...
function cloudDoLogin(email, pw){
  ...
  return Cloud.login(email, pw).then(function(res){
    if(res.cloudState){ STATE=res.cloudState; migrateBudgetV2(); recompute(); render(); toast("Accesso ok · dati dal cloud"); }
...
(async function(){
  ...
  try{
    if(typeof window!=="undefined" && window.HUB_CLOUD && window.supabase && window.supabase.createClient){
      ...
      try{ const res=await Cloud.resume(); if(res && res.cloudState){ STATE=res.cloudState; migrateBudgetV2(); recompute(); render(); } }
