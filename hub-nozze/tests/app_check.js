...
const Sync=(function(){
  ...
  function _push(str){
    ...
    return Promise.resolve(remote.push(baseVersion, str)).then(function(res){
      if(res&&res.ok){ baseVersion=res.version; pendingStr=null; setStatus("synced"); }
        ...
        // CONFLITTO = un altro dispositivo ha avanzato la versione. Politica:
        // Last-Write-Wins DETERMINISTICO sul timestamp _savedAt (non "l'ultimo che
        // pusha"). Se il remoto e' piu' recente -> lo adotto (niente sovrascrittura).
        // Se il locale e' piu' recente (o il remoto non e' confrontabile) -> ripubblico.
        baseVersion=res.version;
        const localTs=savedAt(str), remoteTs=savedAt(res.data);
        if(onConflict){ try{ onConflict(res); }catch(e){} }
        if(remoteTs>localTs){
          // il remoto vince: applico lo stato remoto e non ripubblico il mio
          pendingStr=null; setStatus("synced");
        ...
        // il locale vince: ripubblico sopra la versione avanzata
  ...
  return {
    ...
    enable(r, opts){ remote=r; enabled=!!r; opts=opts||{}; onConflict=opts.onConflict||null; onStatus=opts.onStatus||null; onRemoteWin=opts.onRemoteWin||null; baseVersion=opts.version||0; pendingStr=null; setStatus("synced"); return this; },
...
function seedState(){
  ...
  return {
    ...
    events:{ rb27:{
      ...
      guests:[
        // Bonifica 2026-07-03: rimossi i 10 ospiti di test del seed demo
        // (Marco/Anna/Davide Righi, Giulia Conti, Paolo/Elena/Sofia Biondi,
        // Martina Ferri, Luca Bianchi, Chiara Neri). NOTA per l'utente: i
        // genitori NON sono nella lista dettata — se vanno invitati, aggiungili.
