...
// Nome della macro-voce dell'Analisi intelligente in cui ricade una voce di
// spesa (vuoto se non classificata: resta fuori dai benchmark).
function budgetMacroName(b, vendorsById){
  const id=budgetClassify(b, vendorsById); if(!id) return "";
  const hit=BUDGET_BENCH.find(x=>x.id===id); return hit?hit.name:"";
}
...
function viewBudget(){
  ...
  for(let t=1;t<=5;t++){
    ...
    items.forEach(b=>{
      const macro=budgetMacroName(b, vb);
      rows+=`<tr data-brow="${b.id}">
        <td>${esc(b.item)}${b.paid?' <span class="pill ok">pagata</span>':''}<div class="muted" style="font-size:11px;margin-top:2px">${macro?"↳ "+esc(macro):"<span style=\"opacity:.7\">fuori analisi</span>"}</div></td>
