...
function alertsCompute(){
  ...
  if((e.budget||[]).some(b=>b.costType==="perGuest"&&!b.perHead)) add("bud_perhead","info","Budget","Alcune voci a persona sono senza tariffa.","budget");
...
function viewDash(){
  ...
  <div class="grid cards">
    <div class="card kpi"><div class="v">${money(d.ceiling)}</div><div class="l">Budget massimo (stima + ${m.contingencyPct}% imprevisti)</div></div>
...
(async function(){
  ...
  try{
    if(typeof window!=="undefined" && window.HUB_CLOUD && window.supabase && window.supabase.createClient){
      ...
      try{ const res=await Cloud.resume(); if(res && res.cloudState){ STATE=res.cloudState; recompute(); render(); } }
