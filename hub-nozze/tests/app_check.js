...
// no. Modella il MINIMO GARANTITO del catering (paghi comunque N coperti anche
// se ne confermi meno) sulla voce a coperto più cara (tipicamente il catering).
// Nessun dato modificato: solo stime, etichettate come tali.
function budgetForecast(){
  const e=ev(), m=meta(), lines=e.budget||[];
  let fixedBase=0, rateAll=0, cateringRate=0, cateringName="", quoteSum=0;
  lines.forEach(b=>{
    quoteSum += (+b.quote||0);
    if(b.costType==="perGuest"){
  ...
  return {fixedBase,rateAll,rateOther,cateringRate,cateringName,quoteSum,ceiling,
...
function budgetForecastCard(){
  ...
  return `<div class="card" style="border-left:4px solid var(--${f.verdict==="over"?"no":(f.verdict==="warn"?"gold":(f.verdict==="ok"?"sea":"line"))})">
    ...
    <div class="grid cards" style="grid-template-columns:1fr 1fr 1fr;margin-top:10px">
      ...
      <div class="card kpi"><div class="v">${money(f.cateringRate)}</div><div class="l">Prezzo del catering a coperto <span class="muted" style="font-weight:400">(tariffa a persona, modificabile nella voce)</span></div></div>
    </div>
    ...
    <div class="muted" style="font-size:11px;margin-top:6px">Come leggerla: le voci "a coperto" (catering, navetta…) crescono col numero di persone, le voci fisse (fiori, foto…) no. Il minimo garantito si applica al catering. "A testa" = spesa totale ÷ coperti di quello scenario. Restano stime: i tuoi preventivi sono la verità.</div>
  </div>`;
}
...
function editBudget(id){
  ...
  modal("Modifica voce",
    `<div class="field"><label class="req">Voce</label><input class="inp" id="f_item" value="${esc(b.item)}"></div>
     ...
     <div class="field"><label>Prezzo a coperto (€) <span class="muted" style="font-size:11px">(vuoto = costo fisso; se compilato, la voce scala col numero di ospiti)</span></label><input class="inp" type="number" id="f_perhead" min="0" value="${b.costType==="perGuest"?(b.perHead||""):""}" inputmode="numeric" placeholder="es. 100"></div>
    ...
    [{label:"Annulla"},
     ...
     {label:"Salva",cls:"",fn:()=>{
      ...
      // Prezzo a coperto: >0 -> voce "a coperto" (scala con gli ospiti); vuoto -> costo fisso.
      const ph=Math.max(0, +$("#f_perhead").value||0);
      if(ph>0){ cur.costType="perGuest"; cur.perHead=ph; }
      else if(cur.costType==="perGuest"){ cur.costType="fixed"; cur.perHead=0; }
