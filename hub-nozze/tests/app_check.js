      ...
      Sync.enable(remote, {
        ...
        onRemoteWin:function(st){
          try{
            ...
            if(typeof migrateVendorAddr==="function") migrateVendorAddr();
  ...
  return {
    ...
    events:{ rb27:{
      ...
      vendors:[
        ...
        {id:"benelli",name:"Castello Benelli",category:"Location",contact:"",phone:"",email:"",website:"",address:"Via San Vito 17, Bellaria-Igea Marina (RN)",status:"confermato",quote:7000,budgetLineId:"b0",rating:"",reviews:"",pastEvents:"",notes:"",source:"contratto",updated:""},
        {id:"fregnano",name:"Borgo Fregnano",category:"Location",contact:"",phone:"",email:"",website:"borgofregnano.com",address:"",status:"opzione",quote:4500,budgetLineId:null,rating:"",reviews:"",pastEvents:"",notes:"Opzione presa per il 17/07/2027 (data alternativa a Castello Benelli).",source:"opzione",updated:"2027-07-03"}
      ], tasks:[
...
function recompute(){
  ...
  e.budget.forEach(b=>{
    b._total = (b.costType==="perGuest") ? Math.round((b.perHead||0)*g)
             : (b.costType==="mixed") ? Math.round((+b.fixedBase||0)+(b.perHead||0)*g)
             : b.estimated;
  ...
  const plusAll=e.guests.reduce((s,x)=>s+(+x.plusOne||0),0); // accompagnatori dichiarati in lista
  const plusAttesa=e.guests.filter(x=>x.rsvp==="attesa").reduce((s,x)=>s+(+x.plusOne||0),0);
  ...
  DERIVED={est,quo,act,cont,ceiling,committed,variance:committed-ceiling,cf,paidSum,dueSum,head,counts,meals,intoll,shuttle,kidsN,plusAll,plusAttesa,accessList,plannedGuests:g,tablesN:tablesArr.length,assignedHead,seatCap,vendorsN:vendorsArr.length,vConf,tasksTotal,tasksDone,overdue};
}
...
function alertsCompute(){
  ...
  (function(){
    ...
    Object.keys(hh).forEach(h=>{ if(hh[h].size>1) add("q_hhside_"+qn(h).replace(/\W+/g,""),"info","Ospiti","Nella famiglia \""+h+"\" alcune persone risultano invitate da "+(m.coupleA||"lato A")+" e altre da "+(m.coupleB||"lato B")+" (campo Lato). Di solito è un refuso: tocca per controllare, o silenzia se è voluto.","guests",gs.filter(g=>g.household===h).map(g=>g.id)); });
...
const BUDGET_BENCH=[
  {id:"locat",  name:"Location + Catering", col:"#0C5A63", lo:45, hi:60, kw:/location|castello|villa|borgo|sala|catering|banchett|banqueting|men[uù]|ricevim|aperitiv|buffet/i, cats:["Location","Catering"]},
  {id:"foto",   name:"Foto e video",        col:"#8a7ea6", lo:8,  hi:12, kw:/foto|video|drone|album/i, cats:["Foto/Video"]},
  {id:"musica", name:"Musica e intrattenimento", col:"#c08a87", lo:4, hi:8, kw:/music|dj|band|intratten|anima|spettacol|fuochi/i, cats:["Musica/DJ","Intrattenimento"]},
  {id:"fiori",  name:"Fiori e allestimenti", col:"#6F8268", lo:6, hi:10, kw:/fior|allest|decor|addobb|luci|tovagli/i, cats:["Fiori","Allestimenti"]},
  {id:"abiti",  name:"Abiti e beauty",      col:"#B5852F", lo:8,  hi:12, kw:/abito|vestito|smoking|scarpe|trucco|parruc|beauty|acconciat/i, cats:["Beauty"]},
  {id:"torta",  name:"Torta",               col:"#b58a5a", lo:1,  hi:3,  kw:/torta|cake|dolc|confett/i, cats:["Torta"]},
  {id:"trasp",  name:"Trasporti e navetta", col:"#5aa0a6", lo:1,  hi:3,  kw:/navett|trasport|auto|bus|pullman|limousine/i, cats:["Trasporti/Navetta"]},
  {id:"carta",  name:"Inviti e grafica",    col:"#7a8b99", lo:1,  hi:3,  kw:/invit|partecipaz|grafic|tableau[^ ]* stamp|stamp|segnapost/i, cats:[]},
  {id:"fedi",   name:"Fedi e officiante",   col:"#9c6b4a", lo:2,  hi:4,  kw:/fede|fedi|anell|officiant|celebrant|chiesa|rito|pratiche/i, cats:["Officiante/Pratiche"]}
];
// Classificazione AUTOMATICA di una voce: prima il fornitore collegato, poi le
// parole chiave. Restituisce l'id benchmark o null.
function budgetAutoClassify(b, vendorsById){
  const v=b.vendorId&&vendorsById&&vendorsById[b.vendorId];
...
// Classificazione EFFETTIVA: l'override manuale b.macro vince sull'automatica
// ("__none" = forzata fuori analisi). Usata sia dalla label sia dall'analisi,
// così restano sempre coerenti.
function budgetClassify(b, vendorsById){
  if(b && b.macro){ if(b.macro==="__none") return null; if(BUDGET_BENCH.some(x=>x.id===b.macro)) return b.macro; }
  return budgetAutoClassify(b, vendorsById);
}
// Macro-voce (nome + colore) dell'Analisi intelligente in cui ricade una voce
// di spesa. Se non classificata -> "fuori analisi" con il grigio "nessun gruppo".
function budgetMacroInfo(b, vendorsById){
  const id=budgetClassify(b, vendorsById);
  const hit=id&&BUDGET_BENCH.find(x=>x.id===id);
  return hit ? {id:hit.id, name:hit.name, col:hit.col} : {id:null, name:"fuori analisi", col:"#c9cfd2"};
}
...
function budgetAdvisor(){
  ...
  const rows=BUDGET_BENCH.map(x=>{
    const t=benchTarget(x.id);
    const val=sums[x.id]||0, pct=Math.round(val*100/denom);
    let verdict="—";
    if(val>0) verdict=pct>t.hi?"sopra":(pct<t.lo?"sotto":"ok");
    return {id:x.id, name:x.name, col:x.col, val:val, pct:pct, lo:t.lo, hi:t.hi, verdict:verdict, custom:benchIsCustom(x.id)};
  });
...
/* ============ ANALISI: target benchmark personalizzabili ============ */
// I range % "tipici" (lo-hi) delle macro-voci sono di DEFAULT quelli di prassi
// (BUDGET_BENCH). La coppia può ritararli per il proprio stile: l'override vive
// in e.benchTargets[id]={lo?,hi?} (per-evento), unica fonte per analisi e
// proiezione. Non si CREANO macro-voci nuove (il benchmark è dato di dominio);
// si personalizzano le 9 esistenti.
function benchTarget(id){
  const def=BUDGET_BENCH.find(x=>x.id===id)||{lo:0,hi:0};
  const o=((ev().benchTargets)||{})[id]||{};
  return { lo:(o.lo!=null?+o.lo:def.lo), hi:(o.hi!=null?+o.hi:def.hi) };
}
function benchIsCustom(id){
  const def=BUDGET_BENCH.find(x=>x.id===id)||{lo:0,hi:0}, t=benchTarget(id);
  return t.lo!==def.lo || t.hi!==def.hi;
}
// Salva solo lo scostamento dal default; normalizza 0-100 e lo<=hi.
function benchSet(id, lo, hi){
  const e=ev(); e.benchTargets=e.benchTargets||{};
  const def=BUDGET_BENCH.find(x=>x.id===id); if(!def) return;
  lo=Math.max(0,Math.min(100,Math.round(+lo||0))); hi=Math.max(0,Math.min(100,Math.round(+hi||0)));
  if(hi<lo){ const t=lo; lo=hi; hi=t; }
  const diff={};
  if(lo!==def.lo) diff.lo=lo;
  if(hi!==def.hi) diff.hi=hi;
  if(Object.keys(diff).length) e.benchTargets[id]=diff; else delete e.benchTargets[id];
}
/* ============ fine target benchmark ============ */
/* ---- Meteo (I4): Open-Meteo, gratuito e senza chiave ---- */
// Località di riferimento: l'indirizzo del fornitore LOCATION CONFERMATO
// comanda; in mancanza, l'intestazione evento. Ritorna la città (ultima
// parte dell'indirizzo, senza provincia tra parentesi).
function meteoLocationQuery(){
  const e=ev(), m=meta();
  const v=(e.vendors||[]).find(x=>x.category==="Location"&&x.status==="confermato"&&String(x.address||"").trim());
  const raw=v?v.address:((m&&(m.venueAddr||m.venue))||"");
  ...
  try{
    Object.keys((STATE&&STATE.events)||{}).forEach(k=>{
      ...
      (e.budget||[]).forEach(b=>{
        ...
        const tot=(b.costType==="perGuest") ? Math.round((+b.perHead||0)*g)
                : (b.costType==="mixed") ? Math.round((+b.fixedBase||0)+(+b.perHead||0)*g)
                : (+b.estimated||0);
...
/* ---- Migrazione indirizzo fornitori: campo nuovo v.address. Se il fornitore
   è la location dell'intestazione (stesso nome) eredita l'indirizzo da lì —
   così il meteo della location confermata parte subito. Idempotente: agisce
   solo dove address è ancora undefined. ---- */
function migrateVendorAddr(){
  try{
    let n=0;
    Object.keys((STATE&&STATE.events)||{}).forEach(k=>{
      const e=STATE.events[k]; if(!e) return; const m=e.meta||{};
      (e.vendors||[]).forEach(v=>{
        if(v.address!=null) return;
        v.address=(m.venue&&m.venueAddr&&v.name===m.venue)?m.venueAddr:"";
        n++;
      });
    });
    if(n) Store.save(STATE);
  }catch(err){ Diag.log("vendors","migrazione indirizzi fallita", err&&err.message); }
}
...
// Obbligatori: evidenzia i mancanti (bordo rosso), toast e focus sul primo.
// Uso: reqOk(["#g_name"]) oppure reqOk([["#pm_amount", +$("#pm_amount").value>0]]).
function reqOk(list){
  let first=null;
  list.forEach(it=>{
    const sel=Array.isArray(it)?it[0]:it, el=$(sel);
    const ok=Array.isArray(it)?!!it[1]:!!(el&&el.value.trim());
    if(el) el.classList.toggle("invalid", !ok);
    if(!ok&&!first) first=el;
  });
  if(first){ toast("Compila i campi obbligatori (*)"); try{ first.focus(); }catch(e){} return false; }
  return true;
}
...
function modal(title, bodyHtml, actions){
  ...
  (actions||[{label:"Chiudi",close:true}]).forEach(a=>{
    ...
    b.onclick=()=>{ let r; if(a.fn) r=a.fn(); if(r===false) return; /* validazione fallita: la modale resta aperta */ if(a.close!==false) close(); };
    act.appendChild(b);
  });
...
// Il selettore di lingua è stato rimosso (l'app è solo in italiano: la traduzione
// EN copriva solo i tab, non l'app intera). Questa migrazione riporta a "it" chi
// era rimasto salvato su "en" (così nessuno resta bloccato con i tab in inglese).
function migrateLangReset(){ try{ if(STATE && STATE.lang && STATE.lang!=="it"){ STATE.lang="it"; Store.save(STATE); } }catch(e){} }
/* ==== fine blocco i18n ==== */
...
function exportGuestsCsv(){
  // Solo le variabili compilabili: le derivate (nucleo/lato/gruppo) duplicherebbero
  // le colonne base. L'età esce col valore effettivo (override o da Tipo persona).
  const vars=seatFillableVars();
  ...
  const rows=(ev().guests||[]).map(g=>{
    ...
    return base.concat(vars.map(v=>seatVarValueOf(g,v))).map(q).join(",");
  });
...
function cateringPlanSvg(){
  ...
  tables.forEach(t=>{
    ...
    else if(t.shape==='serpentine'){ shapeSvg=seatBandRectsSvg(t,"#f2f2f2","#444","0.12"); }
...
function buildCateringDoc(){
  ...
  const cards=st.rows.map(r=>{
    const rowsP=r.people.map(p=>{
      ...
      const filaCell=r.long?`<td class="n">${p.fila}</td>`:'';
      return `<tr><td class="n">${p.seat}</td>${filaCell}<td>${esc(p.name)} ${tags}</td><td class="note">${note}</td></tr>`;
    }).join('')||`<tr><td colspan="${r.long?4:3}" class="note">— tavolo vuoto —</td></tr>`;
    const head=r.long?'<th class="n">Posto</th><th class="n">Fila</th><th>Ospite</th><th>Note</th>':'<th class="n">Posto</th><th>Ospite</th><th>Note</th>';
    const hint=r.long?' <span class="sub">· tavolo lungo: sequenza posti dall\'inizio, fila alta/bassa = i due lati</span>':'';
    return `<div class="tc"><h3>${esc(r.name)} <span class="sub">${SEAT_SHAPES[r.shape]||esc(r.shape)} · ${r.people.length}/${r.seats} coperti${r.bambini?' · '+r.bambini+' bambini':''}</span>${hint}</h3>`
      +`<table class="pt${r.long?' long':''}"><thead><tr>${head}</tr></thead><tbody>${rowsP}</tbody></table></div>`;
  }).join('');
  ...
  .pt:not(.long) th:nth-child(3){width:32%}
  .pt.long th:nth-child(2){width:46px}
  .pt.long th:nth-child(4){width:26%}
  ...
  return '<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
    ...
    +unseated
    +(function(){
      const rs=(e.runshow||[]).filter(r=>r.time||r.title).sort((a,b)=>rsTimeKey(a.time)-rsTimeKey(b.time));
      if(!rs.length) return '';
    ...
    +(function(){
      // Snapshot composizione ospiti (confermati): utile per decidere servizi/intrattenimento.
      const G=guestStats("conf"); if(!G.heads) return '';
      const spec=MEALS.filter(k=>k!=="normale").reduce((s,k)=>s+G.meals[k],0);
      const pct=x=>G.heads?Math.round(x*100/G.heads):0;
      const rows=[['Persone', G.heads+(G.plus?' ('+G.invitati+' invitati + '+G.plus+' accompagnatori)':'')],
        ['Adulti / Bambini', G.adulti+' / '+G.bambini+(G.bambini?' ('+pct(G.bambini)+'% bambini)':'')],
        ['Per lato', esc(m.coupleA)+' '+G.lato.A+' · '+esc(m.coupleB)+' '+G.lato.B],
        ['Nuclei', G.nuclei+(G.nucleoMax?' (max '+G.nucleoMax+', '+G.nucleoConBimbi+' con bambini)':'')],
        ['Navetta', G.shuttle+' persone'],
        ['Menù speciali / intolleranze', spec+' / '+G.intoll]];
      const top=G.groupsTop.slice(0,5).map(g=>esc(g[0])+' '+g[1]).join(' · ');
      return '<div class="tables"><h2>Composizione ospiti</h2><table class="mt"><tbody>'
        +rows.map(r=>'<tr><td>'+r[0]+'</td><td class="n">'+r[1]+'</td></tr>').join('')
        +'</tbody></table>'+(top?'<div class="conv">Gruppi principali: '+top+'</div>':'')
        +(G.insights.length?'<div style="margin-top:8px">'+G.insights.map(i=>'<div style="font-size:12px;padding:2px 0">'+i.ic+' '+esc(i.t)+'</div>').join('')+'</div>':'')+'</div>';
    })()
...
async function meteoRefresh(){
  try{
    ...
    const q=meteoLocationQuery(); if(!q) return;
    // location cambiata (nuova confermata o indirizzo modificato): cache invalida
    const fresh=e.meteo&&e.meteo.q===q;
    if(fresh&&e.meteo.ts&&now-e.meteo.ts<12*3600*1000) return;
    ...
    // 1) coordinate della località (riusate finché l'indirizzo non cambia)
    let geo=(fresh&&e.meteo.lat!=null)?{lat:e.meteo.lat,lon:e.meteo.lon,place:e.meteo.place}:null;
    if(!geo){
      const g=await J("https://geocoding-api.open-meteo.com/v1/search?count=1&language=it&name="+encodeURIComponent(q));
    ...
    e.meteo={ts:Date.now(), q:q, lat:geo.lat, lon:geo.lon, place:geo.place, hist:hist, fc:fc};
    Store.save(STATE);
    _meteoBusy=false;
...
function viewDash(){
  ...
  <div class="grid cards">
    ...
    <div class="card kpi" data-act="goTab" data-target="guests" role="link" title="Apri Ospiti & RSVP"><div class="v">${d.counts.conf} <span class="muted" style="font-size:16px">/ ${e.guests.length}${d.plusAll?"+"+d.plusAll:""}</span></div><div class="l">RSVP confermati · in lista ${e.guests.length} ospiti${d.plusAll?" e "+d.plusAll+" accompagnatori":""} · ${d.head} a tavola</div></div>
  ...
  <div class="grid cards">
    ...
    <div class="card kpi" data-act="goTab" data-target="guests" role="link" title="Apri Ospiti & RSVP"><div class="v">${d.counts.attesa}${d.plusAttesa?` <span class="muted" style="font-size:16px">+${d.plusAttesa}</span>`:""}</div><div class="l">In attesa${d.plusAttesa?` (${d.counts.attesa+d.plusAttesa} con +1)`:""}</div></div>
...
function budgetAdvisorCard(){
  ...
  const rows=a.rows.filter(r=>r.val>0||r.lo>=4).map(r=>{
    ...
    return `<tr><td><span class="gdot" style="background:${r.col};margin-right:6px;vertical-align:middle"></span>${esc(r.name)}</td><td class="num">${r.val?money(r.val):"—"}</td><td class="num">${r.val?r.pct+"%":"—"}</td><td class="num muted">${r.lo}–${r.hi}%${r.custom?' <span title="Personalizzato" style="color:var(--sea)">•</span>':""}</td><td><span class="pill ${p[0]}">${p[1]}</span></td></tr>`;
  }).join("");
  return `<div class="card">
    ...
    <div class="muted" style="font-size:11px;margin-top:3px">La colonna "Tipico" sono i target % con cui l'analisi giudica: di default i valori di prassi per l'Italia, ma con "Personalizza" li adatti al vostro stile. I tuoi preventivi restano la verità.</div>
  </div>`;
}
// Editor dei target % (personalizza le 9 macro-voci; non se ne creano di nuove).
function editBenchTargets(){
  const rows=BUDGET_BENCH.map(x=>{ const t=benchTarget(x.id);
    return `<div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--line)">
      <span class="gdot" style="background:${x.col};flex:0 0 auto"></span>
      <span style="flex:1;min-width:0;font-size:13px">${esc(x.name)}</span>
      <input class="inp" type="number" id="bt_lo_${x.id}" value="${t.lo}" min="0" max="100" inputmode="numeric" style="width:56px;padding:6px" aria-label="minimo ${esc(x.name)}">
      <span class="muted">–</span>
      <input class="inp" type="number" id="bt_hi_${x.id}" value="${t.hi}" min="0" max="100" inputmode="numeric" style="width:56px;padding:6px" aria-label="massimo ${esc(x.name)}">
      <span class="muted" style="font-size:12px">%</span>
    </div>`;
  }).join("");
  modal("Personalizza i target %",
    `<p class="muted" style="font-size:12px;margin:-2px 0 6px">Le percentuali sul budget massimo con cui l'analisi giudica "in linea / sopra / sotto" e stima la spesa finale. La somma dei massimi di solito è ~100%. Vuoto/uguali = torna al default.</p>${rows}`,
    [{label:"Ripristina",cls:"ghost",fn:()=>{ ev().benchTargets={}; commit("Target ripristinati"); }},
     {label:"Annulla"},
     {label:"Salva",cls:"",fn:()=>{
        BUDGET_BENCH.forEach(x=>{ benchSet(x.id, $("#bt_lo_"+x.id).value, $("#bt_hi_"+x.id).value); });
        commit("Target aggiornati");
     }}]);
}
/* ============ BUDGET: PROIEZIONE & SCENARI (predittivo) ============ */
// Funzione PURA (testabile): proietta il costo finale in base agli RSVP reali.
// Le voci "a coperto" (costType perGuest) scalano col numero di teste; le fisse
// no. Le voci "miste" (costType mixed: forfait fisso + extra a coperto, tipico
// del catering "totale forfettizzato + extra a persona") contribuiscono con la
// parte fissa alla base e con l'extra alla tariffa a coperto. Modella il MINIMO
// GARANTITO del catering (paghi comunque N coperti anche se ne confermi meno)
// sulla voce a coperto più cara (tipicamente il catering).
// Nessun dato modificato: solo stime, etichettate come tali.
function budgetForecast(){
  const e=ev(), m=meta(), lines=e.budget||[];
  let fixedBase=0, rateAll=0, cateringRate=0, cateringName="", quoteSum=0, forfaitBase=0;
  lines.forEach(b=>{
    quoteSum += (+b.quote||0);
    if(b.costType==="perGuest" || b.costType==="mixed"){
      const ph=+b.perHead||0; rateAll+=ph;
      if(ph>cateringRate){ cateringRate=ph; cateringName=b.item||""; }
      if(b.costType==="mixed"){ const fb=+b.fixedBase||0; fixedBase+=fb; forfaitBase+=fb; }
    } else {
      fixedBase += (+b.actual||0) || (+b.quote||0) || (+b.estimated||0);
    }
  });
  const rateOther=rateAll-cateringRate;
  const heads=arr=>arr.reduce((s,x)=>s+1+(+x.plusOne||0),0);
  const headsConf=heads(e.guests.filter(x=>x.rsvp==="conf"));
  const headsAttesa=heads(e.guests.filter(x=>x.rsvp==="attesa"));
  const planned=+m.plannedGuests||0, minG=+m.minGuaranteed||0;
  // Coperti fatturati per il catering = max(teste, minimo garantito).
  const proj=H=> fixedBase + cateringRate*Math.max(H,minG) + rateOther*H;
  const mk=(key,label,H)=>({key,label,heads:H,cost:proj(H),perHead:H?Math.round(proj(H)/H):0});
  // Più scenari, così il costo si legge anche senza conferme (minimo/previsti).
  const cand=[];
  if(minG>0)          cand.push(mk("min","Minimo garantito catering",minG));
  if(headsConf>0)     cand.push(mk("conf","Solo chi ha già confermato",headsConf));
  if(headsAttesa>0){  cand.push(mk("half","Se metà degli in attesa conferma",headsConf+Math.ceil(headsAttesa/2)));
                      cand.push(mk("all","Se confermano tutti gli invitati",headsConf+headsAttesa)); }
  if(planned>0)       cand.push(mk("planned","Ospiti previsti (pianificazione)",planned));
  // Dedup per numero di teste (tiene la prima etichetta), ordina crescente.
  const seen={}, scenarios=[];
  cand.sort((a,b)=>a.heads-b.heads).forEach(s=>{ if(seen[s.heads])return; seen[s.heads]=1; scenarios.push(s); });
  const worstHeads=scenarios.length?scenarios[scenarios.length-1].heads:Math.max(headsConf,minG);
  const worst=proj(worstHeads);
  const cont=Math.round(quoteSum*(m.contingencyPct||0)/100);
  const ceiling=quoteSum+cont;
  // Costo medio a invitato dello scenario "più realistico" disponibile
  // (previsti > tutti in lista > confermati > minimo): sempre mostrabile.
  const ref=(scenarios.find(s=>s.key==="planned")||scenarios.find(s=>s.key==="all")||scenarios.find(s=>s.key==="conf")||scenarios[scenarios.length-1]||null);
  const perHeadRef= ref? ref.perHead : 0;
  const perHeadRefLabel= ref? ref.label : "";
  const emptyCovers=Math.max(0, minG-headsConf);
  const wasted=Math.round(cateringRate*emptyCovers);
  let verdict="na";
  if(ceiling>0){ const r=worst/ceiling; verdict = r>1?"over":(r>=0.9?"warn":"ok"); }
  return {fixedBase,forfaitBase,rateAll,rateOther,cateringRate,cateringName,quoteSum,ceiling,
    headsConf,headsAttesa,planned,minG,scenarios,worstHeads,worst,perHeadRef,perHeadRefLabel,emptyCovers,wasted,verdict};
}
function budgetForecastCard(){
  const f=budgetForecast();
  if(!f.rateAll && !f.fixedBase) return `<div class="card"><span class="muted" style="font-size:13px">Compila i preventivi (e marca le voci "a coperto") per vedere la proiezione sugli RSVP.</span></div>`;
  const V={ok:["ok","Rientri nel budget"], warn:["warn","Vicino al budget massimo"], over:["no","Rischi di sforare il budget"], na:["todo","Proiezione di spesa"]};
  const v=V[f.verdict]||V.na;
  const band = f.ceiling? ` · budget massimo ${money(f.ceiling)}${f.verdict==="over"?` · <b>${money(f.worst-f.ceiling)} oltre</b>`:""}`:"";
  const scRows=f.scenarios.map(s=>{
    const over=f.ceiling&&s.cost>f.ceiling;
    return `<tr><td>${esc(s.label)}</td><td class="num">${dnum(s.heads)}</td><td class="num">${money(s.cost)}</td><td class="num">${money(s.perHead)}</td><td>${f.ceiling?`<span class="pill ${over?"no":"ok"}">${over?"oltre":"ok"}</span>`:"<span class='muted'>—</span>"}</td></tr>`;
  }).join("");
  return `<div class="card" style="border-left:4px solid var(--${f.verdict==="over"?"no":(f.verdict==="warn"?"gold":(f.verdict==="ok"?"sea":"line"))})">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
      <div><span class="pill ${v[0]}">${v[1]}</span></div>
      <div class="muted" style="font-size:12px">Nello scenario più caro (${dnum(f.worstHeads)} coperti) spendi <b style="color:var(--ink)">${money(f.worst)}</b>${band}</div>
    </div>
    <div class="grid cards" style="grid-template-columns:1fr 1fr 1fr;margin-top:10px">
      <div class="card kpi"><div class="v">${f.rateAll?money(f.rateAll):"—"}</div><div class="l">Quanto costa un invitato in più <span class="muted" style="font-weight:400">(solo voci a coperto)</span></div></div>
      <div class="card kpi"><div class="v">${f.perHeadRef?money(f.perHeadRef):"—"}</div><div class="l">Costo medio a testa, tutto incluso${f.perHeadRefLabel?` <span class="muted" style="font-weight:400">(scenario: ${esc(f.perHeadRefLabel.replace(/\s*\(.*\)/,"").toLowerCase())})</span>`:""}</div></div>
      <div class="card kpi"><div class="v">${money(f.cateringRate)}</div><div class="l">${f.forfaitBase?"Extra a coperto del catering":"Prezzo del catering a coperto"} <span class="muted" style="font-weight:400">(tariffa a persona, modificabile nella voce)</span></div></div>
    </div>
    ${f.forfaitBase?`<div class="card" style="margin-top:2px"><div style="font-size:13px">Contratto <b>misto</b>: forfait fisso <b>${money(f.forfaitBase)}</b> (non cambia col numero di ospiti) <b>+</b> extra <b>${money(f.cateringRate)}</b> a coperto. La stima totale = forfait + extra × coperti.</div></div>`:""}
    ${f.minG?`<div class="card" style="margin-top:2px;${f.emptyCovers>0?'':'opacity:.85'}"><h3 style="font-size:14px">Minimo garantito catering: ${dnum(f.minG)} coperti <span class="muted" style="font-weight:400;font-size:12px">(li paghi anche se confermi meno persone)</span></h3>
      ${f.emptyCovers>0
        ? `<div style="font-size:13px">Confermati finora <b>${dnum(f.headsConf)}</b>, ma paghi comunque <b>${dnum(f.minG)}</b> coperti: <b style="color:var(--no)">${dnum(f.emptyCovers)} pasti pagati a vuoto ≈ ${money(f.wasted)}</b>. Ti servono ancora ${dnum(f.emptyCovers)} conferme per non sprecarli.</div>`
        : `<div class="muted" style="font-size:13px">Confermati ${dnum(f.headsConf)}, sopra il minimo di ${dnum(f.minG)}: nessun pasto pagato a vuoto. ✓</div>`}
    </div>`:""}
    <div class="scroll-x" style="margin-top:8px"><table class="tbl"><thead><tr><th>Se alla fine vengono…</th><th class="num">Coperti</th><th class="num">Spesa totale stimata</th><th class="num">A testa</th><th></th></tr></thead><tbody>${scRows}</tbody></table></div>
    <div class="muted" style="font-size:11px;margin-top:6px">Come leggerla: le voci "a coperto" (catering, navetta…) crescono col numero di persone, le voci fisse (fiori, foto…) no. Le voci "miste" (forfait + extra a persona) hanno una parte fissa e una che scala. Il minimo garantito si applica al catering. "A testa" = spesa totale ÷ coperti di quello scenario. Restano stime: i tuoi preventivi sono la verità.</div>
  </div>`;
}
// Comprimi/espandi le voci di spesa (sono ~50: evita scroll infinito verso i
// riepiloghi in fondo). Preferenza per-dispositivo, non sincronizzata.
let BUDGETLIST_OPEN=(function(){ try{ return localStorage.getItem("hub_budgetlist")!=="0"; }catch(e){ return true; } })();
function toggleBudgetList(){ BUDGETLIST_OPEN=!BUDGETLIST_OPEN; try{ localStorage.setItem("hub_budgetlist", BUDGETLIST_OPEN?"1":"0"); }catch(e){} render(); }
function viewBudget(){
  ...
  for(let t=1;t<=5;t++){
    ...
    items.forEach(b=>{
      const mi=budgetMacroInfo(b, vb);
      rows+=`<tr data-brow="${b.id}">
        <td>${esc(b.item)}${b.paid?' <span class="pill ok">pagata</span>':''}<div style="font-size:11px;margin-top:3px;color:var(--ink-soft)"><span class="gdot" style="background:${mi.col};margin-right:6px;vertical-align:middle"></span>${esc(mi.name)}</div></td>
  ...
  <div class="sec-title"><h2>Voci di spesa</h2><span><button class="btn sm ghost" data-act="toggleBudgetList" title="${BUDGETLIST_OPEN?"Comprimi la lista per scorrere veloce ai riepiloghi":"Mostra le voci"}">${BUDGETLIST_OPEN?"▲ Comprimi":"▼ Espandi"}</button> <button class="btn sm" data-act="addBudget">+ Voce</button></span></div>
  ${BUDGETLIST_OPEN?`
  <div class="scroll-x"><table class="tbl">
    <thead><tr><th>Voce</th><th class="num">Preventivo</th><th class="num">Effettivo</th><th></th></tr></thead>
    <tbody>${rows}</tbody>
  </table></div>`:`
  <div class="card" data-act="toggleBudgetList" role="button" tabindex="0" style="cursor:pointer;text-align:center;padding:16px"><b>${dnum(e.budget.length)} voci di spesa</b> · lista compressa <span class="muted" style="display:block;font-size:12px;margin-top:2px">Tocca per espandere e modificare</span></div>`}
  ...
  <div class="sec-title" style="margin-top:14px"><h2>Proiezione & scenari</h2><span class="pill">predittivo</span></div>
  ${budgetForecastCard()}

  <div class="sec-title" style="margin-top:14px"><h2>Analisi intelligente</h2><span><button class="btn sm ghost" data-act="editBench">Personalizza</button> <span class="pill">${BUDGET_BENCH.some(x=>benchIsCustom(x.id))?"target personalizzati":"benchmark Italia"}</span></span></div>
  ${budgetAdvisorCard()}
  `;
}
...
// Comprimi/espandi la lista ospiti (lista lunga -> evita tap involontari quando
// si scorre ai riepiloghi in fondo). Preferenza per-dispositivo.
let GUESTLIST_OPEN=(function(){ try{ return localStorage.getItem("hub_guestlist")!=="0"; }catch(e){ return true; } })();
function toggleGuestList(){ GUESTLIST_OPEN=!GUESTLIST_OPEN; try{ localStorage.setItem("hub_guestlist", GUESTLIST_OPEN?"1":"0"); }catch(e){} render(); }
function viewGuests(){
  const e=ev(), d=DERIVED;
  const byGroup=GUESTBY==="group";
  ...
  sections.forEach(grp=>{
    ...
    grp.members.forEach(g=>{
      ...
      // etichetta con l'informazione NASCOSTA dalla vista corrente
  ...
  <div class="grid cards">
    ...
    <div class="card kpi"><div class="v">${d.counts.attesa}${d.plusAttesa?` <span class="muted" style="font-size:16px">+${d.plusAttesa}</span>`:""}</div><div class="l">In attesa${d.plusAttesa?` · ${d.counts.attesa+d.plusAttesa} persone con gli accompagnatori`:""}</div></div>
  ...
  <div class="card" style="margin-top:14px"><span class="pill todo">sorgente unica</span> ${ev().guests.length} ospiti in lista${d.plusAll?` + ${d.plusAll} accompagnatori (+1) = <b>${ev().guests.length+d.plusAll} persone</b>`:""}. Con Importa aggiungi in blocco da Excel, Google Sheets o CSV: incolli o carichi il file, mappi le colonne e confermi. Da qui i dati alimentano catering e tavoli.</div>

  <div class="sec-title"><h2>Ospiti per ${byGroup?"gruppo":"nucleo"}</h2><span><button class="btn sm ghost" data-act="toggleGuestList" title="${GUESTLIST_OPEN?"Comprimi la lista per scorrere veloce ai riepiloghi":"Mostra la lista"}">${GUESTLIST_OPEN?"▲ Comprimi":"▼ Espandi"}</button> <button class="btn sm ghost" data-act="shareRsvp">Link RSVP</button> <button class="btn sm ghost" data-act="exportGuestsCsv">Esporta CSV</button> <button class="btn sm ghost" data-act="importGuests">Importa</button> <button class="btn sm" data-act="addGuest">+ Ospite</button></span></div>
  ${GUESTLIST_OPEN?`
  <div style="display:flex;gap:6px;margin-bottom:8px">
    <button class="btn sm ${byGroup?"ghost":""}" data-act="guestsBy" data-by="hh" style="flex:1">Per nucleo</button>
    <button class="btn sm ${byGroup?"":"ghost"}" data-act="guestsBy" data-by="group" style="flex:1">Per gruppo</button>
  </div>
  ...
  `:`
  <div class="card" data-act="toggleGuestList" role="button" tabindex="0" style="cursor:pointer;text-align:center;padding:16px"><b>${ev().guests.length} ospiti</b> · lista compressa <span class="muted" style="display:block;font-size:12px;margin-top:2px">Tocca per espandere e modificare</span></div>
  `}
...
/* ============ COMPOSIZIONE OSPITI (business intelligence) ============ */
// Aggregati puri sulla lista ospiti per capire il parco invitati e decidere su
// intrattenimento/servizi. scope: conf | attesa | all. I "coperti" contano gli
// accompagnatori (+1) come adulti/menù normale/stesso lato/stessa navetta.
let STATS_SCOPE="conf";
let STATS_GROUPS_ALL=false;
function statEta(g){ return (g.attr&&g.attr.eta) || (g.ptype==="bambino"?"Bambino":"Adulto"); }
function statStato(g){ return (g.attr&&g.attr.stato) || ""; }
function guestStats(scope){
  const e=ev(), m=meta();
  const sel=(e.guests||[]).filter(x=> scope==="all"?true : x.rsvp===scope);
  const S={ scope:scope, invitati:sel.length, plus:0, heads:0, bambini:0,
    lato:{A:0,B:0}, eta:{Bambino:0,Giovane:0,Adulto:0,Anziano:0}, stato:{},
    groups:{}, meals:{}, intoll:0, access:0, shuttle:0, rsvp:{conf:0,attesa:0,no:0},
    households:{}, insights:[] };
  MEALS.forEach(k=>S.meals[k]=0);
  sel.forEach(g=>{
    if(S.rsvp[g.rsvp]!=null) S.rsvp[g.rsvp]+=(1+(+g.plusOne||0));
    const w=1+(+g.plusOne||0); S.plus+=(+g.plusOne||0); S.heads+=w;
    S.lato[g.side==="A"?"A":"B"]+=w;
    if(g.ptype==="bambino") S.bambini++;
    const et=statEta(g); S.eta[et]=(S.eta[et]||0)+1; if(g.plusOne) S.eta.Adulto+=(+g.plusOne);
    const grp=g.group||"— senza gruppo"; S.groups[grp]=(S.groups[grp]||0)+w;
    const meal=g.meal||"normale"; S.meals[meal]=(S.meals[meal]||0)+1; if(g.plusOne) S.meals.normale+=(+g.plusOne);
    if(g.intolerances) S.intoll++;
    if(g.accessibility) S.access++;
    if(g.shuttle) S.shuttle+=w;
    const st=statStato(g); if(st) S.stato[st]=(S.stato[st]||0)+1;
    const h=g.household; if(h&&h!=="Senza nucleo"){ (S.households[h]=S.households[h]||[]).push(g); }
  });
  S.adulti=S.heads-S.bambini;
  // nuclei
  const hh=Object.keys(S.households);
  S.nuclei=hh.length;
  S.nucleoMax=hh.reduce((mx,h)=>Math.max(mx,S.households[h].length),0);
  S.nucleoConBimbi=hh.filter(h=>S.households[h].some(g=>g.ptype==="bambino")).length;
  S.senzaNucleo=sel.filter(g=>!g.household||g.household==="Senza nucleo").length;
  // gruppi ordinati
  S.groupsTop=Object.keys(S.groups).map(k=>[k,S.groups[k]]).sort((a,b)=>b[1]-a[1]);
  // --- insight per decisioni ---
  const pct=(x)=> S.heads? Math.round(x*100/S.heads) : 0;
  const add=(ic,t)=>S.insights.push({ic:ic,t:t});
  if(S.heads){
    if(S.bambini>0) add("🧒", S.bambini+" bambini ("+pct(S.bambini)+"%): valuta animazione/baby-sitting, menù e seggioloni, un angolo giochi.");
    const anz=S.eta.Anziano||0; if(anz>0 && pct(anz)>=15) add("👵", anz+" anziani ("+pct(anz)+"%): attenzione a volume musica, accessibilità, orari e navetta.");
    const giov=S.eta.Giovane||0; if(giov>0 && pct(giov)>=25) add("🎉", giov+" giovani ("+pct(giov)+"%): probabile buon riscontro per open bar, DJ e dopocena.");
    if(S.shuttle>0) add("🚌", S.shuttle+" persone useranno la navetta: dimensiona i mezzi (~"+Math.ceil(S.shuttle/50)+" bus da 50).");
    const spec=MEALS.filter(k=>k!=="normale").reduce((s,k)=>s+S.meals[k],0);
    if(spec+S.intoll>0) add("🍽️", spec+" menù speciali e "+S.intoll+" intolleranze: conferma le quantità col catering.");
    const singles=S.stato.Single||0; if(singles>0 && pct(singles)>=25) add("💬", singles+" single: valuta un tavolo/momento di socializzazione o abbinamenti mirati.");
    if(S.groupsTop.length){ const g0=S.groupsTop[0]; if(g0[0].indexOf("senza gruppo")<0 && g0[1]>=8) add("👥", 'Gruppo più numeroso: "'+g0[0]+'" ('+g0[1]+" persone): considera un tavolo/area dedicata."); }
  }
  return S;
}
...
function guestStatsCard(){
  const SC={conf:["ok","solo confermati"], attesa:["warn","solo in attesa"], all:["todo","tutti in lista"]};
  const sc=SC[STATS_SCOPE]?STATS_SCOPE:"conf", lab=SC[sc], S=guestStats(sc), m=meta();
  if(!S.heads) return `<div class="sec-title"><h2>Composizione ospiti</h2><button class="pill ${lab[0]}" data-act="cycleStatsScope" style="cursor:pointer;border:none;font:inherit">${lab[1]} &#8635;</button></div><div class="card"><span class="muted" style="font-size:13px">Nessun dato per questo filtro.</span></div>`;
  const grpColor=k=>k.indexOf("senza gruppo")>=0?"#c9cfd2":seatGroupColor(k);
...
// wNear/wFar alti: le regole esplicite (insieme/lontani) devono dominare sempre
// la similarità pesata (max ~30 con tutte le variabili condivise).
const SEAT_CFG={wNear:40,wFar:50,sideWeight:1.0,faceWeight:0.85,restarts:5,maxSwaps:2000};
// Tetto posti per forma: tondo/quadrato limite fisico ~24; i tavoli lunghi da
// banchetto (rett/imperiale/serpentina) arrivano a 100.
function seatMaxFor(shape){ return (shape==="round"||shape==="square")?24:100; }
...
// Serpentina (P1/P3): tavolo lungo AVVOLTO su più bande (boustrophedon) per
// restare leggibile sul telefono invece di un nastro 16:1. Piccole serpentine
// (fino a ~16 posti) restano in una banda sola (dritte). L'adiacenza NON cambia
// (resta il modello 2-file di seatPairs): l'ottimizzatore e le disposizioni
// esistenti non sono toccati — cambia solo il disegno.
const SEAT_SERP={CG:0.9, RG:0.5, BAND:2.2}; // colonna, mezza-fila, banda (unità mondo)
function seatSegCols(cols){ return cols<=8 ? cols : Math.max(6, Math.ceil(Math.sqrt(cols*1.8))); }
function seatSerpDims(s){
  const cols=Math.max(1,Math.ceil(s/2)), seg=seatSegCols(cols), bands=Math.ceil(cols/seg);
  return {cols, seg, bands};
}
function seatFootprint(t){
  ...
  if(t.shape==='serpentine'){
    const d=seatSerpDims(s), CG=SEAT_SERP.CG, RG=SEAT_SERP.RG, BAND=SEAT_SERP.BAND;
    const bandW=(d.seg-1)*CG;
    return {w:bandW+1.8, h:(d.bands-1)*BAND+2*RG+1.4, lin:bandW};
  }
  const len=Math.max(1.6,Math.ceil(s/2)*0.65); const depth=t.shape==='imperial'?1.0:0.9;
  return {w:len+1.6,h:depth+1.8,lin:len};
}
...
function seatPositions(t){
  ...
  if(t.shape==='serpentine'){
    const d=seatSerpDims(n), CG=SEAT_SERP.CG, RG=SEAT_SERP.RG, BAND=SEAT_SERP.BAND;
    const bandW=(d.seg-1)*CG, y0=((d.bands-1)*BAND)/2;
    for(let i=0;i<n;i++){ const col=Math.floor(i/2), row=i%2;
      const band=Math.floor(col/d.seg), colInBand=col%d.seg;
      const xcol=(band%2===0)?colInBand:(d.seg-1-colInBand); // boustrophedon: righe alternate
      pos.push({x:xcol*CG-bandW/2, y:band*BAND-y0+(row===0?-RG:RG)});
    }
    return pos;
  }
...
// Rettangoli di sfondo per la serpentina avvolta: uno per banda, dimensionato
// sui posti reali della banda (l'ultima banda può essere corta). Usato da
// entrambi i render (app + documento catering).
function seatBandRectsSvg(t,fill,stroke,rx){
  const n=Math.max(0,t.seats|0), d=seatSerpDims(n), CG=SEAT_SERP.CG, RG=SEAT_SERP.RG, BAND=SEAT_SERP.BAND;
  const bandW=(d.seg-1)*CG, y0=((d.bands-1)*BAND)/2, pad=0.55; let svg="";
  for(let b=0;b<d.bands;b++){
    const c0=b*d.seg, c1=Math.min((b+1)*d.seg, d.cols)-1, cnt=c1-c0+1;
    // x-extent reali della banda (tiene conto del boustrophedon)
    let xs=[]; for(let c=c0;c<=c1;c++){ const cib=c%d.seg; xs.push(((b%2===0)?cib:(d.seg-1-cib))*CG-bandW/2); }
    const xmin=Math.min.apply(null,xs), xmax=Math.max.apply(null,xs);
    const cy=b*BAND-y0;
    svg+=`<rect x="${(xmin-pad).toFixed(2)}" y="${(cy-RG-pad).toFixed(2)}" width="${(xmax-xmin+2*pad).toFixed(2)}" height="${(2*RG+2*pad).toFixed(2)}" rx="${rx}" fill="${fill}" stroke="${stroke}" stroke-width="0.05"/>`;
  }
  return svg;
}
// Adiacenze per FORMA: per tonda e quadrata i vicini sono quelli geometrici
// reali del disegno (i 2 posti più vicini), non il modello 2-file — così
// l'ottimizzatore ottimizza esattamente ciò che l'utente vede in planimetria.
// Rettangolare/imperiale/serpentina restano sul modello 2-file (corretto lì).
function seatPairsFor(t){
  const n=Math.max(0,t.seats|0);
  if(t.shape!=='round'&&t.shape!=='square') return seatPairs(n);
  const pos=seatPositions(t), pairs=[], seen=new Set();
  const add=(a,b,w)=>{ const k=Math.min(a,b)+'-'+Math.max(a,b); if(seen.has(k))return; seen.add(k); pairs.push({a:Math.min(a,b),b:Math.max(a,b),w:w}); };
  for(let i=0;i<n;i++){
    const d=[]; for(let j=0;j<n;j++){ if(j!==i){ const dx=pos[i].x-pos[j].x, dy=pos[i].y-pos[j].y; d.push([j,dx*dx+dy*dy]); } }
    d.sort((x,y)=>x[1]-y[1]);
    for(let k=0;k<Math.min(2,d.length);k++) add(i,d[k][0],SEAT_CFG.sideWeight);
  }
  return pairs;
}
...
// simFn(ga,gb) -> numero: similarità pesata reale (A3). Le regole esplicite
// dominano (wNear/wFar >> similarità massima). pairs opzionale: adiacenze
// precomputate per forma (seatPairsFor); default modello 2-file.
// Contributo di UNA coppia adiacente al costo (fonte unica: usata sia dal costo
// totale sia dal costo incrementale). Regole esplicite dominano la similarità.
function seatPairCost(order,pr,rulesIdx,simFn){
  const ga=order[pr.a],gb=order[pr.b];
  if(ga===undefined||gb===undefined) return 0;
  const key=ga<gb?ga+'|'+gb:gb+'|'+ga;
  if(rulesIdx.separate.has(key)) return SEAT_CFG.wFar*pr.w;
  if(rulesIdx.together.has(key)) return -SEAT_CFG.wNear*pr.w;
  if(simFn) return -simFn(ga,gb)*pr.w;
  return 0;
}
function seatCost(order,n,rulesIdx,simFn,pairs){
  pairs=pairs||seatPairs(n); let cost=0;
  for(const pr of pairs) cost+=seatPairCost(order,pr,rulesIdx,simFn);
  return cost;
}
// 2-opt multi-restart. Migliorie: (a) budget scambi PER restart scalato su n²
// (i restart casuali non vengono più affamati); (b) costo INCREMENTALE — uno
// scambio (i,j) tocca solo le coppie incidenti a i o j, quindi ricalcolo solo
// quelle (O(grado)) invece dell'intero costo (O(n)). Il test di equivalenza
// verifica che after == seatCost(order) esatto.
function seatOptimize(gids,seats,rulesIdx,simFn,pairs){
  pairs=pairs||seatPairs(seats);
  const people=gids.filter(x=>x!=null);
  const baseOrder=new Array(seats).fill(undefined); people.forEach((g,i)=>baseOrder[i]=g);
  const before=seatCost(baseOrder,seats,rulesIdx,simFn,pairs);
  // indice posizione -> coppie incidenti (per il costo incrementale)
  const p2p=[]; for(let i=0;i<seats;i++) p2p.push([]);
  pairs.forEach(pr=>{ p2p[pr.a].push(pr); p2p[pr.b].push(pr); });
  // somma dei contributi delle coppie toccate da i o j (dedup: la coppia {i,j}
  // sta in entrambe le liste; un flag di generazione evita di contarla due volte).
  const seen=new Int32Array(pairs.length); let gen=0;
  const touchedCost=(order,i,j)=>{ gen++; let s=0;
    const li=p2p[i]; for(let k=0;k<li.length;k++){ const pr=li[k]; if(seen[pr._i]!==gen){ seen[pr._i]=gen; s+=seatPairCost(order,pr,rulesIdx,simFn); } }
    const lj=p2p[j]; for(let k=0;k<lj.length;k++){ const pr=lj[k]; if(seen[pr._i]!==gen){ seen[pr._i]=gen; s+=seatPairCost(order,pr,rulesIdx,simFn); } }
    return s; };
  pairs.forEach((pr,k)=>pr._i=k); // id coppia per il dedup
  const budgetPer=Math.max(SEAT_CFG.maxSwaps, seats*seats*2); // ~ passi completi anche a n grande
  let bestOrder=baseOrder.slice(),bestCost=before;
  for(let r=0;r<SEAT_CFG.restarts;r++){
    ...
    let curCost=seatCost(cur,seats,rulesIdx,simFn,pairs);
    let swapBudget=budgetPer,improved=true;
    while(improved&&swapBudget>0){ improved=false;
      for(let i=0;i<seats&&swapBudget>0;i++){ for(let j=i+1;j<seats&&swapBudget>0;j++){
        ...
        swapBudget--;
        const b=touchedCost(cur,i,j);
        const ti=cur[i]; cur[i]=cur[j]; cur[j]=ti;
        const a=touchedCost(cur,i,j);
        const delta=a-b;
        if(delta<-1e-9){ curCost+=delta; improved=true; }
...
// v3 (B1): le variabili con 'src' derivano il valore LIVE dal campo nativo
// dell'ospite (side/household/ptype/group): niente doppia compilazione, niente
// dati divergenti. 'override:true' (età) permette di raffinare il valore in attr.
const SEAT_VARS_DEF=[
  {id:'nucleo',   name:'Nucleo / Famiglia', on:true,  weight:10, mode:'cluster', src:'household', values:['']},
  {id:'lato',     name:'Lato',              on:true,  weight:5,  mode:'cluster', src:'side', values:['']},
  {id:'eta',      name:"Fascia d'età",      on:true,  weight:4,  mode:'cluster', src:'ptype', override:true, values:['','Bambino','Giovane','Adulto','Anziano']},
  {id:'ambiente', name:'Gruppo / Ambiente', on:true,  weight:6,  mode:'cluster', src:'group', values:['']},
...
const SEAT_VARS_VER=3; // v2: +stato · v3: nucleo/lato/età/ambiente derivate dai campi nativi
function seatVars(){
  ...
  if(e.seating.varsVer<3){
    // le 4 variabili doppione diventano derivate; gli attr copiati (ora ridondanti)
    // vengono rimossi — per l'età si conservano i raffinamenti Giovane/Anziano.
    const src={nucleo:'household', lato:'side', eta:'ptype', ambiente:'group'};
    e.seating.vars.forEach(v=>{ if(src[v.id]){ v.src=src[v.id]; if(v.id==='eta') v.override=true; if(v.id==='ambiente') v.name='Gruppo / Ambiente'; } });
    (e.guests||[]).forEach(g=>{ if(!g.attr) return;
      delete g.attr.nucleo; delete g.attr.lato; delete g.attr.ambiente;
      if(g.attr.eta==='Bambino'||g.attr.eta==='Adulto') delete g.attr.eta;
    });
    e.seating.varsVer=3;
  }
...
// Variabili che si compilano a mano (senza sorgente nativa, o con override).
function seatFillableVars(){ return seatActiveVars().filter(v=>!v.src||v.override); }
// Valore EFFETTIVO di una variabile per un ospite: campo nativo se derivata
// (con eventuale override in attr per l'età), altrimenti attr.
function seatVarValueOf(g,v){
  if(!g) return "";
  const ov=(g.attr&&g.attr[v.id])||"";
  if(v.src==='side') return g.side||"";
  if(v.src==='household'){ const h=g.household; return (h&&h!=='Senza nucleo')?h:""; }
  if(v.src==='ptype') return ov||(g.ptype==='bambino'?'Bambino':'Adulto');
  if(v.src==='group') return g.group||"";
  return ov;
}
function seatVarCoverage(v){ const ppl=seatableGuests(); let n=0; ppl.forEach(g=>{ if(seatVarValueOf(g,v)) n++; }); return {n:n, tot:ppl.length}; }
// Similarità pesata con segno (G2): stesso valore -> +peso (unisci) o -peso (separa).
// Copre anche nucleo/gruppo/bambini via variabili derivate (niente casi speciali).
function seatSimilarity(a,b){ let s=0; if(!a||!b) return 0; seatActiveVars().forEach(v=>{ const av=seatVarValueOf(a,v), bv=seatVarValueOf(b,v); if(!av||!bv) return; if(av===bv){ s+= v.mode==='separate'? -v.weight : v.weight; } }); return s; }
function seatSimilarityFn(){ const by={}; ev().guests.forEach(g=>by[g.id]=g); return function(ga,gb){ return seatSimilarity(by[ga],by[gb]); }; }
/* ---- segnaposto accompagnatori +1 (C4) ---- */
// Persone "sedibili" = ospiti + segnaposto per i plusOne (id: gid#pK), se attivi.
function seatPlusEnabled(){ const e=ev(); return !(e.seating&&e.seating.plus1===false); }
function seatOwnerId(pid){ const i=(pid||"").indexOf("#p"); return i<0?pid:pid.slice(0,i); }
function seatCompanionsOf(g){
  const out=[], n=Math.max(0,+g.plusOne||0);
  for(let k=1;k<=n;k++) out.push({id:g.id+"#p"+k, name:g.name+" +"+k, side:g.side, household:g.household, group:g.group, ptype:"adulto", attr:g.attr, _plusOf:g.id});
  return out;
}
function seatPeople(){
  const gs=seatableGuests(); if(!seatPlusEnabled()) return gs.slice();
  const out=gs.slice(); gs.forEach(g=>{ seatCompanionsOf(g).forEach(c=>out.push(c)); });
  return out;
}
function seatPersonById(pid){
  const own=seatOwnerId(pid), g=(ev().guests||[]).find(x=>x.id===own);
  if(!g) return null;
  if(own===pid) return g;
  const k=+pid.slice(own.length+2)||0;
  if(!seatPlusEnabled()||k<1||k>(+g.plusOne||0)) return null;
  return seatCompanionsOf(g)[k-1];
}
// Vincoli impliciti: ogni segnaposto +N va accanto al suo titolare.
function seatRulesIdxAll(){
  const idx=seatRulesIdx(); if(!seatPlusEnabled()) return idx;
  seatableGuests().forEach(g=>{ seatCompanionsOf(g).forEach(c=>{ idx.together.add(c.id<g.id?c.id+"|"+g.id:g.id+"|"+c.id); }); });
  return idx;
}
// simFn per persone: i segnaposto ereditano le caratteristiche del titolare.
function seatPersonSimFn(){
  const by={}; ev().guests.forEach(g=>by[g.id]=g);
  return function(pa,pb){ const oa=seatOwnerId(pa), ob=seatOwnerId(pb); if(oa===ob) return 15; return seatSimilarity(by[oa],by[ob]); };
}
// Posti occupati da id non più validi (ospite eliminato, +1 ridotti, segnaposto
// disattivati): li libera. Chiamata dal render Tavoli; salva senza commit.
function seatSweepStale(){
  let n=0; (ev().tables||[]).forEach(t=>{ if(!t.seatIds) return; t.seatIds=t.seatIds.map(id=>{ if(!id) return null; if(seatPersonById(id)) return id; n++; return null; }); });
  return n;
}
...
function seatPrintStats(){
  ...
  const isLong=s=>(s==='serpentine'||s==='imperial'||s==='rect');
  tables.forEach(t=>{
    const row={id:t.id,name:t.name,shape:t.shape,seats:t.seats,long:isLong(t.shape),people:[],bambini:0,plus:0,intoll:[],acc:[],meals:{}};
    ...
    (t.seatIds||[]).forEach((id,i)=>{ if(!id) return; const p=seatPersonById(id); if(!p) return;
      ...
      // tavoli lunghi (modello 2-file): posto pari = fila alta, dispari = fila bassa
      const fila=isLong(t.shape)?(i%2===0?'alta':'bassa'):'';
      row.people.push({seat:i+1,fila:fila,name:p.name,plus:isPlus,kid:kid,meal:meal,intol:isPlus?'':(p.intolerances||''),acc:isPlus?'':(p.accessibility||'')});
...
function renderPlanimetria(){
  ...
  tables.forEach(t=>{
    ...
    else if(t.shape==='serpentine'){ shapeSvg=seatBandRectsSvg(t,"#e9eef0","#9bb0b3","0.15"); }
    ...
    pos.forEach((p,i)=>{
      ...
      seats+=`<g transform="translate(${p.x.toFixed(2)},${p.y.toFixed(2)})" data-act="assignSeat" data-table="${t.id}" data-idx="${i}" data-seat-target="1"${g?` data-drag-guest="${gid}"`:""} style="cursor:pointer;touch-action:none">`
        ...
        +(g?`<text x="0" y="0.12" text-anchor="middle" font-size="0.34" fill="#23373b">${esc(lbl)}</text>`:"")
        +`</g>`;
    });
  ...
  modal(isNew?"Nuovo tavolo":"Modifica tavolo",
    `<div class="field"><label>Nome</label><input class="inp" id="tb_name" value="${esc(cur.name)}"></div>
     <div class="two">
       ...
       <div class="field"><label>Posti <span class="muted" style="font-size:11px">(tondo/quadrato max 24, tavoli lunghi max 100)</span></label><input class="inp" type="number" min="1" max="100" id="tb_seats" value="${cur.seats}"></div>
     </div>`,
    [{label:"Annulla"},{label:isNew?"Crea":"Salva",cls:"",fn:()=>{
       const name=($("#tb_name").value||"").trim()||"Tavolo";
       const shape=$("#tb_shape").value, cap=seatMaxFor(shape);
       const want=+$("#tb_seats").value||8, seats=Math.max(1,Math.min(cap,want));
       if(want>cap) toast("Massimo "+cap+" posti per la forma "+(SEAT_SHAPES[shape]||shape).toLowerCase());
       if(isNew){
...
function togglePlusSeats(){
  const e=ev(); e.seating=e.seating||{rules:[]};
  const turnOn=(e.seating.plus1===false);
  e.seating.plus1=turnOn?true:false;
  if(!turnOn) seatSweepStale(); // spegnendo, libera i posti dei segnaposto
  commit(turnOn?"Segnaposto +1 attivati":"Segnaposto +1 disattivati");
}
...
function seatGuestName(gid){ const g=seatPersonById(gid); return g?g.name:"(ospite rimosso)"; }
...
function assignSeat(tableId, idx){
  ...
  const cand=seatPeople().filter(g=>!seated.has(g.id)||g.id===occupied);
...
function wireSeating(){
  ...
  const nameOf=gid=>{ const g=seatPersonById(gid); return g?g.name:"ospite"; };
  function cleanup(){
...
// Peso a 3 livelli chiari (B3): Poco=3, Normale=6, Molto=10. I pesi numerici
// storici restano validi: il livello mostrato è quello più vicino.
function seatWLevelName(w){ return w>=8?'Molto':(w>=5?'Normale':'Poco'); }
function seatWizVarRow(v,i){
  const cov=seatVarCoverage(v), lvl=seatWLevelName(v.weight);
  const src=v.src?(v.override?'dai dati ospiti, raffinabile':'dai dati ospiti'):'da compilare al passo 2';
  return `<div class="varrow"><button type="button" class="switch${v.on?' on':''}" data-vtog="${i}" aria-pressed="${v.on?'true':'false'}" aria-label="Attiva ${esc(v.name)}"></button>`
    +`<div class="vn">${esc(v.name)}<span class="muted" style="font-size:11px;display:block">${src} · ${cov.n}/${cov.tot}</span></div>`
    +`<select class="inp" style="width:88px;padding:4px" data-vw="${i}" title="Quanto conta per l'abbinamento">${[['3','Poco'],['6','Normale'],['10','Molto']].map(o=>`<option value="${o[0]}"${lvl===o[1]?' selected':''}>${o[1]}</option>`).join('')}</select>`
    +`<select class="inp" style="width:88px;padding:4px" data-vm="${i}" title="Unisci avvicina chi condivide il valore, separa li allontana"><option value="cluster"${v.mode!=='separate'?' selected':''}>unisci</option><option value="separate"${v.mode==='separate'?' selected':''}>separa</option></select>`
    +(v.custom?`<button type="button" class="btn sm danger" data-vdel="${i}" aria-label="Elimina">&times;</button>`:'')+`</div>`;
}
function seatWizVars(){
  const vars=seatVars(), der=[], fil=[];
  vars.forEach((v,i)=>{ (v.src?der:fil).push([v,i]); });
  let h='<p class="muted" style="font-size:13px">L\'algoritmo abbina gli ospiti con queste caratteristiche. Quelle "dai dati ospiti" (nucleo, lato, età, gruppo) sono già pronte: nessuna doppia compilazione. "Quanto conta": Molto = quasi sempre allo stesso tavolo.</p>';
  if(der.length) h+='<div class="muted" style="font-size:11px;letter-spacing:.4px;text-transform:uppercase;margin:8px 0 2px">Dai dati ospiti — automatiche</div>'+der.map(x=>seatWizVarRow(x[0],x[1])).join('');
  h+='<div class="muted" style="font-size:11px;letter-spacing:.4px;text-transform:uppercase;margin:10px 0 2px">Extra — si compilano al passo 2</div>'
    +(fil.length?fil.map(x=>seatWizVarRow(x[0],x[1])).join(''):'<p class="muted" style="font-size:12px">Nessuna.</p>');
  h+='<div style="text-align:right;margin:6px 0 2px"><button type="button" class="btn sm ghost" id="wz-prefill" title="Deduce Single/In coppia/Famiglia da nuclei e accompagnatori">Deduci lo Stato dai dati</button></div>';
...
// % compilazione: solo sulle variabili che si compilano davvero a mano
// (quelle derivate sono sempre "piene" dai dati; l'età-override è facoltativa).
function seatAttrFillPct(){ const vars=seatActiveVars().filter(v=>!v.src), people=seatableGuests(); const cells=vars.length*people.length; if(!cells) return 100; let f=0; people.forEach(g=>vars.forEach(v=>{ if(g.attr&&g.attr[v.id]) f++; })); return Math.round(f*100/cells); }
...
// (Nucleo/lato/età/ambiente non si copiano più: sono derivate live — B1.)
function seatPrefillAttrs(){
  ...
  Store.save(STATE); toast(n?("Dedotto lo Stato di "+n+" invitati"):"Stato già compilato per tutti");
}
function seatWizFill(){
  const vars=seatFillableVars();
  if(!vars.length) return '<div class="muted" style="font-size:13px">Tutte le caratteristiche attive derivano dai dati ospiti: qui non c\'è nulla da compilare. Avanti.</div>'+seatWizNav(2);
  ...
  if(v.override) h+='<p class="muted" style="font-size:12px">Facoltativa: chi resta senza valore usa il Tipo persona (Adulto/Bambino). Compila solo Giovane/Anziano dove serve.</p>';
...
function seatWizFillBind(host){
  ...
  if(!seatFillableVars().length) return; // niente da agganciare
...
function seatRulesOutcome(){
  ...
  let ok=0; const bad=[];
  rules.forEach(r=>{ const same=tof[r.a]&&tof[r.a]===tof[r.b]; const good=r.kind==='together'?same:!same;
    if(good)ok++; else bad.push(esc(seatGuestName(r.a))+(r.kind==='together'?' doveva stare con ':' doveva stare lontano da ')+esc(seatGuestName(r.b))); });
  let h=`<div style="font-size:13px;margin-top:4px">Regole rispettate: ${ok}/${rules.length}</div>`;
  if(bad.length) h+='<div style="font-size:12px;color:var(--no);margin-top:2px">'+bad.slice(0,6).join('<br>')+(bad.length>6?'<br>… e altre '+(bad.length-6):'')+'</div>';
  return h;
}
// Composizione per tavolo (C2): chi c'è, riassunto per gruppo/nucleo.
function seatTablesCompo(){
  const rows=(ev().tables||[]).filter(t=>seatHeadAt(t)>0).map(t=>{
    const cnt={}; (t.seatIds||[]).forEach(id=>{ if(!id)return; const own=seatGuestById(seatOwnerId(id)); if(!own)return; const k=own.group||((own.household&&own.household!=='Senza nucleo')?own.household:'Altro'); cnt[k]=(cnt[k]||0)+1; });
    const parts=Object.keys(cnt).sort((a,b)=>cnt[b]-cnt[a]).slice(0,3);
    const extra=Object.keys(cnt).length-parts.length;
    return `<div style="font-size:12px;padding:2px 0;border-bottom:1px solid var(--line)"><b>${esc(t.name)}</b> ${seatHeadAt(t)}/${t.seats}: ${parts.map(k=>esc(k)+' ×'+cnt[k]).join(' · ')}${extra>0?' · +'+extra:''}</div>`;
  });
  return rows.length?'<div style="margin-top:8px">'+rows.join('')+'</div>':'';
}
function seatWizGen(){
...
function viewSeating(){
  const tables=ev().tables||[];
  // integrità: libera i posti con id non più validi (ospite eliminato, +1 ridotti)
  if(seatSweepStale()>0) Store.save(STATE);
  ...
  const people=seatPeople();
  const unseated=people.filter(g=>!seatedSet.has(g.id));
  const plusOn=seatPlusEnabled(), nPlus=people.filter(p=>p._plusOf).length;
  const tray=`<div id="seatTray" data-tray="1" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;padding:8px;border:1px dashed var(--line);border-radius:10px;min-height:38px">`
    +(unseated.length?unseated.map(g=>`<span class="tag" data-drag-guest="${g.id}" style="cursor:grab;touch-action:none${g._plusOf?';opacity:.75;font-style:italic':''}">${esc(g.name)} (${g.side})</span>`).join("")
      :`<span class="muted" style="font-size:13px">Tutte le persone da sedere sono assegnate.</span>`)+`</div>`;
  return `
  <div class="card"><span class="pill todo">come funziona</span> Crea i tavoli con + Tavolo, poi premi <b>Genera (guidato)</b>: in 4 passi scegli le caratteristiche che contano (nucleo, lato, età e gruppo sono già presi dai dati ospiti), compili solo quelle extra, aggiungi i vincoli insieme/lontani e generi la disposizione — con Annulla se non convince. Gli accompagnatori (+1) hanno un segnaposto accanto al loro invitato. Ritocchi a mano: tocca un posto o trascina un nome dalla riserva; trascina un tavolo per spostarlo.</div>
  <div class="grid cards">
    ...
    <div class="card kpi"><div class="v">${unseated.length}</div><div class="l">Da sedere${plusOn&&nPlus?` (con ${nPlus} +1)`:''}</div></div>
  </div>
  ...
  ${tables.length?`<div class="tbar">
    ...
    <button class="btn sm ghost" data-act="togglePlus" title="Segnaposto per gli accompagnatori">+1: ${plusOn?'Sì':'No'}</button>
...
function vendorDeadlinesCard(){
  ...
  // Ordina per SCADENZA crescente (la più vicina in alto): è uno scadenzario,
  // non un elenco per categoria. A parità di data, ordine alfabetico.
  const byDate=(a,b)=> (a.deadline||"").localeCompare(b.deadline||"") || a.cat.localeCompare(b.cat);
  const shown=dls.filter(dl=>!dl.off||DEC_SHOWHIDDEN).sort(byDate), hidden=dls.filter(dl=>dl.off).sort(byDate);
...
function editVendor(id){
  ...
  modal(isNew?"Nuovo fornitore":"Modifica fornitore",
    `<div class="field"><label class="req">Nome</label><input class="inp" id="v_name" value="${esc(x.name)}"></div>
     <div class="two">
     ...
     <div class="field"><label>Indirizzo <span class="muted" style="font-size:11px">(per Location confermata guida anche il meteo)</span></label><input class="inp" id="v_addr" value="${esc(x.address||"")}" placeholder="Via, Città (Prov)"></div>
     <div class="two">
     ...
     ...(isNew?[]:[{label:"Elimina",cls:"danger",fn:()=>{ setTimeout(()=>delVendor(id),0); }}]),
     {label:"Salva",cls:"",fn:()=>{   if(!reqOk(["#v_name"])) return false; const name=$("#v_name").value.trim();
      const data={name,category:$("#v_cat").value,status:$("#v_st").value,contact:$("#v_ref").value.trim(),phone:$("#v_ph").value.trim(),email:$("#v_em").value.trim(),website:$("#v_web").value.trim(),address:$("#v_addr").value.trim(),quote:+$("#v_q").value||0,rating:$("#v_rt").value.trim(),budgetLineId:$("#v_bl").value,optionUntil:$("#v_opt").value||"",reviews:$("#v_rev").value.trim(),pastEvents:$("#v_pe").value.trim(),notes:$("#v_nt").value.trim()};
      let vv;
      const E=ev();
...
function viewTimeline(){
  ...
  const rs=[...e.runshow].sort((a,b)=>rsTimeKey(a.time)-rsTimeKey(b.time)).filter(r=>!rsFilter||r.who===rsFilter);
  ...
  <div class="sec-title"><h2>Scaletta</h2><span><button class="btn sm ghost" data-act="shareScaletta" title="Link di sola lettura per gli altri organizzatori">Condividi</button> <button class="btn sm ghost" data-act="printScaletta">Stampa</button> <button class="btn sm ghost" data-act="genRunShow">Scaletta standard</button> <button class="btn sm" data-act="addRs">+ Momento</button></span></div>
...
function editTask(id){
  ...
  modal(isNew?"Nuova attività":"Modifica attività",
    `<div class="field"><label class="req">Attività</label><input class="inp" id="t_title" value="${esc(x.title)}"></div>
    ...
    [{label:"Annulla"},{label:"Salva",cls:"",fn:()=>{
      if(!reqOk(["#t_title"])) return false; const title=$("#t_title").value.trim();
...
function editRs(id){
  ...
  modal(isNew?"Nuovo momento":"Modifica momento",
    `<div class="two"><div class="field"><label>Ora</label><input class="inp" type="time" id="r_time" value="${esc(x.time||"")}"></div>
     ...
     <div class="field"><label class="req">Momento</label>${managedSelect("r_title", x.title||"", [""].concat(RUNSHOW_SUGGEST))}</div>
    ...
    [{label:"Annulla"},{label:"Salva",cls:"",fn:()=>{
      const title=(managedValue("r_title")||"").trim(); if(!title){ toast("Scegli o scrivi il momento"); return false; }
...
function addList(){
  modal("Nuova lista",`<div class="field"><label class="req">Titolo</label><input class="inp" id="nl_t" placeholder="Es. Lista regali testimoni"></div>`,
   [{label:"Annulla"},{label:"Crea",cls:"",fn:()=>{ if(!reqOk(["#nl_t"])) return false; const t=$("#nl_t").value.trim(); ev().lists.push({id:"l"+Date.now(),title:t,items:[]}); commit("Lista creata"); }}]);
}
...
function editBudget(id){
  ...
  const vb={}; (ev().vendors||[]).forEach(v=>vb[v.id]=v);
  const autoId=budgetAutoClassify(b, vb), autoHit=autoId&&BUDGET_BENCH.find(x=>x.id===autoId);
  const autoLbl=autoHit?autoHit.name:"fuori analisi";
  modal("Modifica voce",
    `<div class="field"><label class="req">Voce</label><input class="inp" id="f_item" value="${esc(b.item)}"></div>
     <div class="two">
     ...
     <div class="two">
       <div class="field"><label>Prezzo a coperto (€) <span class="muted" style="font-size:11px">(vuoto = costo fisso; se compilato, scala con gli ospiti)</span></label><input class="inp" type="number" id="f_perhead" min="0" value="${(b.costType==="perGuest"||b.costType==="mixed")?(b.perHead||""):""}" inputmode="numeric" placeholder="es. 100"></div>
       <div class="field"><label>Forfait base (€) <span class="muted" style="font-size:11px">(parte fissa del contratto misto: non scala)</span></label><input class="inp" type="number" id="f_fixedbase" min="0" value="${b.costType==="mixed"?(b.fixedBase||""):""}" inputmode="numeric" placeholder="es. 3000"></div>
     </div>
     ...
     <div class="field"><label>Macro-voce <span class="muted" style="font-size:11px">(analisi intelligente)</span></label>
       <select class="inp" id="f_macro">
         <option value=""${!b.macro?" selected":""}>Automatica · ${esc(autoLbl)}</option>
         ${BUDGET_BENCH.map(x=>`<option value="${x.id}"${b.macro===x.id?" selected":""}>${esc(x.name)}</option>`).join("")}
         <option value="__none"${b.macro==="__none"?" selected":""}>Fuori analisi</option>
       </select></div>`,
    [{label:"Annulla"},
     // setTimeout: la conferma va aperta DOPO che close() ha svuotato #modalRoot.
     ...
     {label:"Salva",cls:"",fn:()=>{
      ...
      if(!reqOk(["#f_item"])) return false;
      const cur=ev().budget.find(x=>x.id===id);
      if(!cur){ toast("Voce non più presente (aggiornata da un altro dispositivo)"); return; }
      cur.item=$("#f_item").value.trim();
      cur.quote=+$("#f_quote").value||0; cur.actual=+$("#f_act").value||0; cur.paid=$("#f_paid").value==="1";
      // Modello di costo: a coperto (scala), misto (forfait fisso + extra a coperto), fisso.
      const ph=Math.max(0, +$("#f_perhead").value||0);
      const fb=Math.max(0, +$("#f_fixedbase").value||0);
      if(ph>0){ cur.perHead=ph; if(fb>0){ cur.costType="mixed"; cur.fixedBase=fb; } else { cur.costType="perGuest"; cur.fixedBase=0; } }
      else { if(cur.costType==="perGuest"||cur.costType==="mixed"){ cur.costType="fixed"; } cur.perHead=0; cur.fixedBase=0; }
      const mv=$("#f_macro").value; if(mv) cur.macro=mv; else delete cur.macro; // "" = automatica
...
function addBudget(){
  modal("Nuova voce",
    `<div class="field"><label class="req">Voce</label><input class="inp" id="n_item" placeholder="Es. Noleggio lounge"></div>
    ...
    [{label:"Annulla"},{label:"Aggiungi",cls:"",fn:()=>{
      if(!reqOk(["#n_item"])) return false; const item=$("#n_item").value.trim();
...
function editPayment(id){
  ...
  modal(isNew?"Nuova rata":"Modifica rata",
    `<div class="field"><label class="req">Descrizione</label><input class="inp" id="pm_label" value="${esc(x.label||"")}" placeholder="Es. Acconto catering"></div>
     <div class="two">
       <div class="field"><label class="req">Importo (€)</label><input class="inp" type="number" id="pm_amount" value="${x.amount||0}" min="0"></div>
    ...
    [{label:"Annulla"},{label:isNew?"Aggiungi":"Salva",cls:"",fn:()=>{
       if(!reqOk(["#pm_label",["#pm_amount", +$("#pm_amount").value>0]])) return false;
      const data={label:($("#pm_label").value||"").trim(), amount:+$("#pm_amount").value||0, dueDate:($("#pm_due").value||"").trim(), method:$("#pm_method").value, vendorId:$("#pm_vendor").value||null, paid:$("#pm_paid").value==="1"};
       const E=ev(); E.payments=E.payments||[];
...
function seatGuestAttrFields(x){
  const vars=seatFillableVars(); if(!vars.length) return "";
  const attr=x.attr||{};
  const miss=vars.filter(v=>!v.override&&!attr[v.id]).length;
  const rows=vars.map(v=>`<div class="field"><label>${esc(v.name)}${v.override?' <span class="muted" style="font-size:11px">(da Tipo persona se vuoto)</span>':''}</label><select class="inp" data-gattr="${v.id}">${v.values.map(val=>`<option value="${esc(val)}"${(attr[v.id]||"")===val?" selected":""}>${val===""?"—":esc(val)}</option>`).join("")}</select></div>`).join("");
  return `<details style="margin-top:6px"><summary class="muted" style="cursor:pointer">Caratteristiche per i tavoli${miss?` — <b style="color:var(--gold)">${miss} da compilare</b>`:" ✓"}</summary><div class="two" style="margin-top:6px">${rows}</div></details>`;
}
...
function editGuest(id){
  ...
  modal(isNew?"Nuovo ospite":"Modifica ospite",
    `<div class="field"><label class="req">Nome</label><input class="inp" id="g_name" value="${esc(x.name)}"></div>
     <div class="two">
  ...
  inp.onchange=()=>{ const f=inp.files[0]; if(!f) return; const r=new FileReader();
    r.onload=()=>{ try{ const s=JSON.parse(r.result);
      if(s&&s.events){
        ...
        try{ openEvents(); }catch(e){} // torna alla lista aggiornata
...
function openEvents(){
  ...
  modal("I miei eventi",
    rows+`<div style="text-align:center;margin:6px 0"><button class="btn" data-act="evNew">+ Nuovo evento</button></div>
    <div style="border-top:1px solid var(--line);margin-top:10px;padding-top:10px;display:flex;gap:6px;justify-content:center;flex-wrap:wrap">
      <button class="btn sm ghost" data-act="evBackupAll">Backup completo (tutti)</button>
      <button class="btn sm ghost" data-act="evImport">Importa evento o backup</button>
    </div>
    <p class="muted" style="font-size:11px;margin-top:8px">Ogni evento è indipendente e resta archiviato qui. <b>Esporta</b> (su una riga) salva il singolo evento; <b>Backup completo</b> salva tutti gli eventi e le impostazioni in un file; <b>Importa</b> ricarica un evento o un backup unendolo a quelli presenti. I dati stanno solo su questo dispositivo: un backup periodico è la tua rete di sicurezza.</p>`,
    [{label:"Chiudi"}]);
}
...
function cloudDoLogin(email, pw){
  ...
  return Cloud.login(email, pw).then(function(res){
    if(res.cloudState){ STATE=res.cloudState; migrateBudgetV2(); migrateSeedLabels(); migrateTaskVendorCat(); migrateGuestsRoster2(); migrateMealSplit(); migrateVendorAddr(); migrateLangReset(); recompute(); render(); toast("Accesso ok · dati dal cloud"); }
...
/* ---- Scaletta condivisibile in sola lettura (autocontenuta nell'URL) ----
   Il cloud è dormiente, quindi il link non è "vivo" lato server: la scaletta
   viaggia CODIFICATA nell'URL (base64url UTF-8 safe). Chi apre il link vede una
   pagina pulita di sola lettura, su qualunque dispositivo, senza login. È uno
   snapshot: se cambi la scaletta rigeneri il link. ---- */
function encodeScaletta(obj){
  const json=JSON.stringify(obj);
  return btoa(unescape(encodeURIComponent(json))).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"");
}
function decodeScaletta(str){
  try{ let b=String(str).replace(/-/g,"+").replace(/_/g,"/"); while(b.length%4) b+="=";
    return JSON.parse(decodeURIComponent(escape(atob(b)))); }catch(e){ return null; }
}
function scalettaParam(){ try{ const m=(location.search||"").match(/[?&]scaletta=([^&]+)/); return m?m[1]:null; }catch(e){ return null; } }
// Ordinamento "giorno delle nozze": le ore dopo mezzanotte (prima delle 06:00)
// appartengono alla notte successiva, quindi vanno IN FONDO (01:00 dopo 23:00).
// Orari mancanti/invalidi in coda. (Hoisted: usata anche dalla vista Timeline.)
function rsTimeKey(t){ const m=String(t||"").match(/^(\d{1,2}):(\d{2})/); if(!m) return 1e9; let mins=(+m[1])*60+(+m[2]); if(+m[1]<6) mins+=1440; return mins; }
// Payload compatto della scaletta corrente (chiavi corte per URL brevi).
function scalettaData(){
  const e=ev(), m=meta();
  const rows=[...((e&&e.runshow)||[])].filter(r=>r.time||r.title).sort((a,b)=>rsTimeKey(a.time)-rsTimeKey(b.time))
    .map(r=>({t:r.time||"", m:r.title||"", w:r.who||"", p:(typeof musicUrl==="function"?musicUrl(r.playlist):r.playlist)||""}));
  return { a:m.coupleA||"", b:m.coupleB||"", d:m.date||"", v:m.venue||"", r:rows };
}
...
// Pagina pubblica read-only della scaletta (decodificata dall'URL).
function renderPublicScaletta(data){
  try{ const tb=document.getElementById("tabs"); if(tb) tb.style.display="none"; }catch(e){}
  try{ const gb=document.getElementById("gearBtn"); if(gb) gb.style.display="none"; const sc=document.getElementById("syncChip"); if(sc) sc.style.display="none"; const ab=document.getElementById("alertBell"); if(ab) ab.style.display="none"; }catch(e){}
  try{ const br=document.querySelector(".brand"); if(br){ br.removeAttribute("data-act"); br.style.cursor="default"; } const pn=document.getElementById("editPencil"); if(pn) pn.style.display="none"; }catch(e){}
  const v=document.getElementById("view"); if(!v) return;
  if(!data||!data.r){ v.innerHTML='<div class="card" style="max-width:560px;margin:16px auto"><h2>Scaletta non leggibile</h2><p class="muted" style="font-size:13px">Il link sembra incompleto. Chiedi di rigenerarlo.</p></div>'; return; }
  try{ const et=document.getElementById("evTitle"); if(et) et.textContent=((data.a||"")+(data.b?" × "+data.b:""))||"Le nostre nozze"; const es=document.getElementById("evSub"); if(es) es.textContent=(data.v?data.v+" · ":"")+(data.d?fdate(data.d):""); const cd=document.getElementById("cd"); if(cd&&cd.parentNode) cd.parentNode.style.display="none"; }catch(e){}
  const rows=(data.r||[]).map(function(r){
    const pl=r.p?'<div style="margin-top:3px"><a href="'+esc(r.p)+'" target="_blank" rel="noopener" class="pill todo" style="text-decoration:none;display:inline-block">&#9654; playlist</a></div>':"";
    return '<tr><td class="num" style="white-space:nowrap;font-weight:600">'+esc(r.t||"—")+'</td><td>'+esc(r.m||"")+pl+'</td><td class="muted">'+esc(r.w||"")+'</td></tr>';
  }).join("")||'<tr><td colspan="3" class="muted">Scaletta vuota.</td></tr>';
  v.innerHTML='<div class="card" style="max-width:560px;margin:16px auto">'
    +'<div class="sec-title" style="margin-top:2px"><h2>Scaletta del giorno</h2><span class="pill todo">sola lettura</span></div>'
    +'<div class="scroll-x"><table class="tbl"><thead><tr><th>Ora</th><th>Momento</th><th>Chi</th></tr></thead><tbody>'+rows+'</tbody></table></div>'
    +'<p class="muted" style="font-size:12px;margin-top:10px">Versione di sola lettura condivisa dagli sposi. Gli aggiornamenti richiedono un nuovo link.</p></div>';
}
// Documento stampabile della scaletta (PDF/carta).
function buildScalettaDoc(){
  const d=scalettaData();
  const rows=d.r.map(function(r){ return '<tr><td class="n">'+esc(r.t||"—")+'</td><td>'+esc(r.m||"")+(r.p?' <span class="pl">'+esc(r.p)+'</span>':"")+'</td><td>'+esc(r.w||"")+'</td></tr>'; }).join("")||'<tr><td colspan="3">Scaletta vuota.</td></tr>';
  const css='@page{size:A4;margin:16mm}body{font-family:Georgia,\'Times New Roman\',serif;color:#1a1a1a;margin:0;padding:20px}.head{text-align:center;border-bottom:2px solid #1a1a1a;padding-bottom:10px;margin-bottom:16px}.head h1{font-size:24px;font-weight:600;margin:0}.head .sub{font-size:13px;color:#444;margin-top:4px}table{border-collapse:collapse;width:100%;font-size:13px}th,td{border:1px solid #bbb;padding:6px 8px;text-align:left;vertical-align:top}th{background:#efefef}td.n{width:70px;white-space:nowrap;font-weight:600}.pl{display:block;font-size:11px;color:#555;word-break:break-all}.toolbar{text-align:center;margin:0 0 14px}.toolbar button{font-size:15px;padding:8px 22px;cursor:pointer}@media print{.toolbar{display:none}}';
  return '<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Scaletta — '+esc(d.a)+' e '+esc(d.b)+'</title><style>'+css+'</style></head><body>'
    +'<div class="toolbar"><button onclick="window.print()">Stampa / Salva PDF</button></div>'
    +'<div class="head"><h1>Scaletta del giorno</h1><div class="sub">'+esc(d.a)+(d.b?' e '+esc(d.b):"")+(d.d?' — '+esc(fdate(d.d)):"")+(d.v?'<br>'+esc(d.v):"")+'</div></div>'
    +'<table><thead><tr><th class="n">Ora</th><th>Momento</th><th>Responsabile</th></tr></thead><tbody>'+rows+'</tbody></table></body></html>';
}
function shareScaletta(){
  const obj=scalettaData();
  if(!obj.r.length){ toast("Aggiungi prima qualche momento alla scaletta"); return; }
  const url=location.origin+location.pathname+"?scaletta="+encodeScaletta(obj);
  showShareLink("Scaletta da condividere (sola lettura)", url, "Ecco la scaletta del giorno:");
}
function printScaletta(){
  const obj=scalettaData(); if(!obj.r.length){ toast("Scaletta vuota"); return; }
  const w=window.open("","_blank"); if(!w){ toast("Consenti i popup per stampare"); return; }
  w.document.write(buildScalettaDoc()); w.document.close();
}
/* ============ EVENTS ============ */
document.addEventListener("click",e=>{ try{
  ...
  else if(act==="shareScaletta") shareScaletta();
  else if(act==="printScaletta") printScaletta();
  ...
  else if(act==="editBench") editBenchTargets();
  ...
  else if(act==="evBackupAll"){ exportEvent(); }
  else if(act==="evImport"){ importEvent(); }
  ...
  else if(act==="togglePlus") togglePlusSeats();
...
(async function(){
  ...
  if(scalettaParam()){ renderPublicScaletta(decodeScaletta(scalettaParam())); return; }
  ...
  migrateLangReset(); // selettore lingua rimosso: forza IT
  migrateVendorAddr(); // campo indirizzo fornitori (eredita da intestazione se location)
  ...
  try{
    if(typeof window!=="undefined" && window.HUB_CLOUD && window.supabase && window.supabase.createClient){
      ...
      try{ const res=await Cloud.resume(); if(res && res.cloudState){ STATE=res.cloudState; migrateBudgetV2(); migrateSeedLabels(); migrateTaskVendorCat(); migrateGuestsRoster2(); migrateMealSplit(); migrateVendorAddr(); migrateLangReset(); recompute(); render(); } }
