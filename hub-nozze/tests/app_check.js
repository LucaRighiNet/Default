...
function budgetForecast(){
  ...
  const scenarios=[
    {key:"conf",   label:"Confermati",  heads:headsConf,             cost:proj(headsConf)},
    {key:"attesa", label:"+ In attesa", heads:headsConf+headsAttesa, cost:proj(headsConf+headsAttesa)},
    {key:"planned",label:"Previsti",    heads:planned,               cost:proj(planned)}
  ];
  const worstHeads=Math.max(headsConf+headsAttesa, planned);
  const worst=proj(worstHeads);
  const cont=Math.round(quoteSum*(m.contingencyPct||0)/100);
  const ceiling=quoteSum+cont;
  const perHeadReal= headsConf? Math.round(proj(headsConf)/headsConf) : 0;
  ...
  return {fixedBase,rateAll,rateOther,cateringRate,cateringName,quoteSum,ceiling,
    headsConf,headsAttesa,planned,minG,scenarios,worstHeads,worst,perHeadReal,emptyCovers,wasted,verdict};
}
function budgetForecastCard(){
  ...
  const scRows=f.scenarios.map(s=>{
    ...
    return `<tr><td>${esc(s.label)}</td><td class="num">${dnum(s.heads)}</td><td class="num">${money(s.cost)}</td><td>${f.ceiling?`<span class="pill ${over?"no":"ok"}">${over?"oltre":"ok"}</span>`:"<span class='muted'>—</span>"}</td></tr>`;
  }).join("");
  return `<div class="card" style="border-left:4px solid var(--${f.verdict==="over"?"no":(f.verdict==="warn"?"gold":(f.verdict==="ok"?"sea":"line"))})">
    ...
    <div class="grid cards" style="grid-template-columns:1fr 1fr;margin-top:10px">
      <div class="card kpi"><div class="v">${f.headsConf?money(f.perHeadReal):"—"}</div><div class="l">Costo per invitato confermato (tutto incluso)</div></div>
    ...
    <div class="scroll-x" style="margin-top:8px"><table class="tbl"><thead><tr><th>Scenario RSVP</th><th class="num">Coperti</th><th class="num">Costo proiettato</th><th></th></tr></thead><tbody>${scRows}</tbody></table></div>
    <div class="muted" style="font-size:11px;margin-top:6px">Stima: voci "a coperto" scalate sulle teste (accompagnatori inclusi), fisse invariate, minimo garantito sul catering. I preventivi restano la verità.</div>
  </div>`;
}
function viewBudget(){
