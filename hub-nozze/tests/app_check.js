...
/* ============ STORE (backend rilevato: window.storage | localStorage | RAM) ============ */
const KEY="hub_state_v1";
let memState=null, saveTimer=null;
// Feature detection, NON user-agent. window.storage e' iniettata solo dal
// runtime artifact di Claude.ai; fuori (browser desktop, Safari iPhone,
// Playwright) non esiste e si ricade su localStorage, che persiste tra reload.
// RAM = ultima spiaggia (nessuna persistenza). Vedi handoff sez.5 opzione B.
const Backend=(function(){
  ...
  try{
    if(typeof localStorage!=="undefined"){
      const probe="__hub_probe__"; localStorage.setItem(probe,"1"); localStorage.removeItem(probe);
      return {
        kind:"localStorage",
  ...
  return { kind:"memory", async get(){ return null; }, async set(){} };
})();
const Store={
  backend:Backend.kind,
  async load(){
    try{ const v=await Backend.get(); if(v) return JSON.parse(v); }catch(e){}
    return memState;
  },
  save(state){
    memState=state;
    clearTimeout(saveTimer);
    saveTimer=setTimeout(async()=>{ try{ await Backend.set(JSON.stringify(state)); }catch(e){} },250);
  },
  // Scrittura immediata di una save in sospeso: chiude la finestra di 250ms in cui
  // una chiusura/reload perderebbe l'ultima modifica. Con localStorage la set è
  // sincrona, quindi persiste anche durante l'unload.
  flush(){
    if(saveTimer){ clearTimeout(saveTimer); saveTimer=null; }
    if(memState!=null){ try{ Backend.set(JSON.stringify(memState)); }catch(e){} }
  }
};
