...
function guestStats(scope){
  ...
  const S={ scope:scope, invitati:sel.length, plus:0, heads:0, bambini:0,
    ...
    groups:{}, meals:{}, intoll:0, access:0, shuttle:0,
    households:{}, insights:[] };
...
function guestStatsCard(){
  ...
  <div class="grid cards">
    ...
    <div class="card kpi"><div class="v">${S.nuclei}</div><div class="l">Nuclei${S.nucleoMax?` · max ${S.nucleoMax}`:""}</div></div>
  </div>
  <div class="grid" style="grid-template-columns:1fr 1fr;gap:14px">
    ...
    <div class="card" style="grid-column:1 / -1"><h3>Gruppi</h3>
      ${S.groupsTop.slice(0,8).map(g=>statBar(g[0], g[1], S.heads, grpColor(g[0]))).join("")}
      ${S.groupsTop.length>8?`<div class="muted" style="font-size:12px">…e altri ${S.groupsTop.length-8} gruppi</div>`:""}</div>
...
const READONLY_ACTS={ openGuide:1, openAccount:1, openDiag:1, hideTip:1, exportGuestsCsv:1, printTables:1, seatZoomIn:1, seatZoomOut:1, seatZoomReset:1, openAlerts:1, goAlert:1, togglePrivacy:1, goTab:1, cycleCateringScope:1, cycleStatsScope:1 };
