...
function merge3(base, a, b, aTs, bTs){
  const J=x=>JSON.stringify(x);
  if(J(a)===J(b)) return a;
  if(J(a)===J(base)) return b;   // ha cambiato solo il remoto
  if(J(b)===J(base)) return a;   // ha cambiato solo il locale
  ...
  if(isObj(a)&&isObj(b)){
    ...
    keys.forEach(function(k){
      ...
      if(inA&&inB){ out[k]=merge3(bk,a[k],b[k],aTs,bTs); }
  ...
  if((isIdArr(a)||isIdArr(b)) && Array.isArray(a) && Array.isArray(b)){
    ...
    mA.forEach(function(ea,id){
      ...
      if(eb!==undefined){ put(id, merge3(e0,ea,eb,aTs,bTs)); }
