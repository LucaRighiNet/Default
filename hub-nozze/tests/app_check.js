...
function budgetForecast(){
  ...
  const cand=[];
  if(minG>0)          cand.push(mk("min","Minimo garantito",minG));
  if(headsConf>0)     cand.push(mk("conf","Confermati",headsConf));
  if(headsAttesa>0){  cand.push(mk("half","Metà attesa conferma",headsConf+Math.ceil(headsAttesa/2)));
                      cand.push(mk("all","Tutti gli invitati",headsConf+headsAttesa)); }
  if(planned>0)       cand.push(mk("planned","Previsti (pianificazione)",planned));
...
function budgetForecastCard(){
  ...
  const V={ok:["ok","In linea col budget"], warn:["warn","Vicino al tetto"], over:["no","Rischio sforamento"], na:["todo","Proiezione"]};
  const v=V[f.verdict]||V.na;
  const band = f.ceiling? ` · tetto ${money(f.ceiling)}${f.verdict==="over"?` · <b>${money(f.worst-f.ceiling)} oltre</b>`:""}`:"";
  ...
  return `<div class="card" style="border-left:4px solid var(--${f.verdict==="over"?"no":(f.verdict==="warn"?"gold":(f.verdict==="ok"?"sea":"line"))})">
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
      ...
      <div class="muted" style="font-size:12px">Proiezione peggiore (${dnum(f.worstHeads)} coperti): <b style="color:var(--ink)">${money(f.worst)}</b>${band}</div>
    </div>
    <div class="grid cards" style="grid-template-columns:1fr 1fr 1fr;margin-top:10px">
      <div class="card kpi"><div class="v">${f.rateAll?money(f.rateAll):"—"}</div><div class="l">Costo di ogni invitato in più (a coperto)</div></div>
      <div class="card kpi"><div class="v">${f.perHeadRef?money(f.perHeadRef):"—"}</div><div class="l">Costo medio a invitato, tutto incluso${f.perHeadRefLabel?` · ${esc(f.perHeadRefLabel.replace(/\s*\(.*\)/,"").toLowerCase())}`:""}</div></div>
      <div class="card kpi"><div class="v">${money(f.cateringRate)}</div><div class="l">Catering a coperto${f.cateringName?` · ${esc(f.cateringName)}`:""}</div></div>
    </div>
    ${f.minG?`<div class="card" style="margin-top:2px;${f.emptyCovers>0?'':'opacity:.85'}"><h3 style="font-size:14px">Minimo garantito catering: ${dnum(f.minG)} coperti</h3>
      ${f.emptyCovers>0
        ? `<div style="font-size:13px">Confermati ora <b>${dnum(f.headsConf)}</b> → paghi comunque <b>${dnum(f.minG)}</b>: <b style="color:var(--no)">${dnum(f.emptyCovers)} coperti a vuoto ≈ ${money(f.wasted)}</b>. Servono ancora ${dnum(f.emptyCovers)} conferme per non sprecarli.</div>`
        : `<div class="muted" style="font-size:13px">Confermati ${dnum(f.headsConf)} ≥ minimo: nessun coperto pagato a vuoto. ✓</div>`}
    </div>`:""}
    <div class="scroll-x" style="margin-top:8px"><table class="tbl"><thead><tr><th>Scenario RSVP</th><th class="num">Coperti</th><th class="num">Costo proiettato</th><th class="num">€/invitato</th><th></th></tr></thead><tbody>${scRows}</tbody></table></div>
    <div class="muted" style="font-size:11px;margin-top:6px">Stima: voci "a coperto" scalate sulle teste (accompagnatori inclusi), fisse invariate, minimo garantito sul catering. "€/invitato" = costo medio tutto incluso a quel numero di coperti. I preventivi restano la verità.</div>
  </div>`;
}
