...
function seatCost(order,n,rulesIdx,simFn,pairs){
  ...
  for(const pr of pairs){ const ga=order[pr.a],gb=order[pr.b];
    ...
    else if(simFn)cost-=simFn(ga,gb)*pr.w;
  }
  return cost;
}
function seatOptimize(gids,seats,rulesIdx,simFn,pairs){
  pairs=pairs||seatPairs(seats); // precompute: seatCost gira ~2000 volte
  ...
  const evalOrder=ord=>seatCost(ord,seats,rulesIdx,simFn,pairs);
